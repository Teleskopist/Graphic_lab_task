...
using LiteMath::uint, LiteMath::uint2, LiteMath::clamp, LiteMath::lerp, LiteMath::int4, LiteMath::min, LiteMath::max, LiteMath::float2, LiteMath::float3, LiteMath::float4, LiteMath::float4x4, LiteMath::M_TWOPI, LiteMath::M_PI, LiteMath::INV_PI, LiteMath::make_float2, LiteMath::make_float3, LiteMath::complex, LiteMath::DEG_TO_RAD;
#endif
...
static inline float3 decodeNormal(unsigned int a_data)
{
  const uint32_t enc_x = a_data & 0x0000FFFF;
  const uint32_t enc_y = a_data >> 16;

  const float sign = ((enc_x & 1u) != 0) ? -1.0f : 1.0f;

  // sign-extend 16-bit values (with bit 0 of enc_x cleared) to 32-bit signed int
  // equivalent of (short)(enc_x & 0xFFFE) and (short)(enc_y) but without short
  const int ix = int(((enc_x & 0xFFFEu) ^ 0x8000u) - 0x8000u);
  const int iy = int((enc_y           ^ 0x8000u) - 0x8000u);

  const float x = float(ix) * (1.0f / 32767.0f);
  const float y = float(iy) * (1.0f / 32767.0f);
