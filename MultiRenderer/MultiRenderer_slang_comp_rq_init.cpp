...
{
  auto pObj = std::make_shared<MultiRenderer_slang_comp_rq>(AS_type, maxPrimitives);
  pObj->SetVulkanContext(a_ctx);
  pObj->InitVulkanObjects(a_ctx.device, a_ctx.physicalDevice, a_maxThreadsGenerated);
  return pObj;
}

vk_utils::VulkanDeviceFeatures MultiRenderer_slang_comp_rq_ListRequiredDeviceFeatures()
{
  vk_utils::VulkanDeviceFeatures res;
  res.features2 = MultiRenderer_slang_comp_rq::ListRequiredDeviceFeatures(res.extensionNames);
  res.apiVersion = VK_API_VERSION_1_2;
  return res;
}

void MultiRenderer_slang_comp_rq::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
  ...
  m_pAccelStruct = std::make_shared<RTX_Proxy>(m_pAccelStructOld, m_pAccelStruct, true); // wrap both user and RTX implementation with proxy object 
}
...
VkBufferUsageFlags MultiRenderer_slang_comp_rq::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t MultiRenderer_slang_comp_rq::GetDefaultMaxTextures() const { return 256; }

void MultiRenderer_slang_comp_rq::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void MultiRenderer_slang_comp_rq::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
void MultiRenderer_slang_comp_rq::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
  vkDestroyBuffer(m_device, m_classDataBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.m_pAccelStruct_remapBuffer, nullptr);
...
MultiRenderer_slang_comp_rq::~MultiRenderer_slang_comp_rq()
{
...
void MultiRenderer_slang_comp_rq::InitHelpers()
{
  vkGetPhysicalDeviceProperties(m_physicalDevice, &m_devProps);
}


void MultiRenderer_slang_comp_rq::InitKernel_ResolveDebug(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_ResolveDebug.comp.spv");
...
void MultiRenderer_slang_comp_rq::InitKernel_FillGbuffer(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_FillGbuffer.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_FillGbufferWithTransparency(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_FillGbufferWithTransparency.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_ResolveOutline(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_ResolveOutline.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_AverageColor(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_AverageColor.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_Tonemap(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_Tonemap.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_PackXY(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel2D_PackXY.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_ResolveCommon(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_ResolveCommon.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_ResolveCommonWithTransparency(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_ResolveCommonWithTransparency.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernel_ResolveCTF(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang_comp_rq/kernel1D_ResolveCTF.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
...
void MultiRenderer_slang_comp_rq::InitKernels(const char* a_filePath)
{
...
void MultiRenderer_slang_comp_rq::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
...
void MultiRenderer_slang_comp_rq::ReserveEmptyVectors()
{
  if(m_pAccelStruct_remap.capacity() == 0)
    m_pAccelStruct_remap.reserve(4);
...
void MultiRenderer_slang_comp_rq::InitDeviceData()
{
  ...
  memberVectors.push_back(m_classDataBuffer);
  {
    ...
    m_pAccelStruct_remap = std::vector(tablePtrs.table, tablePtrs.table + tablePtrs.tableSize);
  }
  m_vdata.m_pAccelStruct_remapBuffer = vk_utils::createBuffer(m_device, m_pAccelStruct_remap.capacity()*sizeof(uint2), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.m_pAccelStruct_remapBuffer);
...
VkImage MultiRenderer_slang_comp_rq::CreateTexture2D(const int a_width, const int a_height, VkFormat a_format, VkImageUsageFlags a_usage)
{
...
VkSampler MultiRenderer_slang_comp_rq::CreateSampler(const Sampler& a_sampler) // TODO: implement this function correctly
{
...
VkImageView MultiRenderer_slang_comp_rq::CreateView(VkFormat a_format, VkImage a_image)
{
...
void MultiRenderer_slang_comp_rq::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[MultiRenderer_slang_comp_rq::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
MultiRenderer_slang_comp_rq::MemLoc MultiRenderer_slang_comp_rq::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
MultiRenderer_slang_comp_rq::MemLoc MultiRenderer_slang_comp_rq::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "MultiRenderer_slang_comp_rq::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void MultiRenderer_slang_comp_rq::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void MultiRenderer_slang_comp_rq::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 MultiRenderer_slang_comp_rq::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
MultiRenderer_slang_comp_rq::MegaKernelIsEnabled MultiRenderer_slang_comp_rq::m_megaKernelFlags;
