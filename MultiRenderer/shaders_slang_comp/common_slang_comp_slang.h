...
static const uint GRAPH_PRIM_POINT = 0u;
static const uint GRAPH_PRIM_LINE = 1u;
static const uint GRAPH_PRIM_LINE_SEGMENT = 2u;
static const uint GRAPH_PRIM_LINE_SEGMENT_DIR = 3u;
static const uint GRAPH_PRIM_BOX = 4u;
static const uint GRAPH_PRIM_POINT_COLOR = 5u;
static const uint GRAPH_PRIM_LINE_COLOR = 6u;
static const uint GRAPH_PRIM_LINE_SEGMENT_COLOR = 7u;
static const uint GRAPH_PRIM_LINE_SEGMENT_DIR_COLOR = 8u;
static const uint GRAPH_PRIM_BOX_COLOR = 9u;
...
static const uint SBS_IN_SIDE_X_NEG = 0u;
static const uint SBS_IN_SIDE_X_POS = 1u;
static const uint SBS_IN_SIDE_Y_NEG = 2u;
static const uint SBS_IN_SIDE_Y_POS = 3u;
static const uint SBS_IN_SIDE_Z_NEG = 4u;
static const uint SBS_IN_SIDE_Z_POS = 5u;
static const uint SDF_SBS_NODE_LAYOUT_UNDEFINED = 0 << 24u;
static const uint SDF_SBS_NODE_LAYOUT_DX = 1 << 24u;
static const uint SDF_SBS_NODE_LAYOUT_DX_UV16 = 2 << 24u;
static const uint SDF_SBS_NODE_LAYOUT_DX_RGB8 = 3 << 24u;
static const uint SDF_SBS_NODE_LAYOUT_ID32F_IRGB32F = 4 << 24u;
static const uint SDF_SBS_NODE_LAYOUT_ID32F_IRGB32F_IN = 5 << 24u;
static const uint SDF_SBS_NODE_LAYOUT_MASK = 0xFF000000;
...
static const uint MULTI_OCTREE_FLAG_SURFACE = 1;
struct SdfMultiOctreeNode
{
  uint data_offset;    // offset in data vector for all distance values, offset is in uint, not bytes 
  uint children_offset;// offset for children (they are stored together). 0 offset means it's a leaf
  uint voxel_count;    // number of voxels in this node, 0 mean it does not contain surface
  uint flags;          // enum SdfMultiOctreeFlags
};
static const uint COCTREE_NODE_PACK_MODE_DEFAULT = 0;
static const uint COCTREE_NODE_PACK_MODE_SIM_COMP_FULL = 1;
static const uint COCTREE_NODE_PACK_MODE_SIM_COMP_SMALL = 2;
static const uint COCTREE_LEAF_TYPE_NOT_A_LEAF = 0;
static const uint COCTREE_LEAF_TYPE_GRID = 1;
static const uint COCTREE_LEAF_TYPE_BIT_PACK = 2;
static const uint COCTREE_LEAF_TYPE_SLICES = 3;
static const uint COCTREE_LEAF_TYPE_BITS = 2;
static const uint COCTREE_LEAF_TYPE_MASK = 0x3;
static const uint COCTREE_LOD_LEAF_TYPE_SHIFT = 30;
static const uint COCTREE_MAX_CHILD_INFO_SIZE = 2;
static const uint COCTREE_USE_BEST_LEAF_TYPE = 1000;
...
static const uint TAG_NONE = 0;
static const uint TAG_TRIANGLE = 1;
static const uint TAG_COCTREE_BRICKED = 2;
static const uint TAG_SDF_BRICK = 4;
static const uint TAG_SDF_NODE = 8;
static const uint TAG_GRAPHICS_PRIM = 9;
static const uint TAG_SDF_GRID = 11;
static const uint TAG_OPENVDB_GRID = 14;
static const uint TAG_SDF_OCTREE = 15;
static const uint TAG_CSGDNF = 16;
static const uint TAG_SDF_DAG = 18;
static const uint TAG_SCOM2 = 19;
...
static const uint m_lod_palette[20] = {
...
struct MultiRenderer_slang_comp_UBO_Data
{
  float4x4 m_projInv; 
...
bool is_compact_type(uint type) ;
float2 encode_normal(float3 v) ;
...
float4 rayCapsuleIntersect(in float3 ray_pos, in float3 ray_dir, in float3 pa, in float3 pb, float ra) ;
...
float get_max_sdf_val(float level_sz) ;
...
uint get_links_end(in SCom2Header header, uint child_n, uint node1, uint node2) ;
bool DAG_extract_is_surface(uint voxel_count_flags) ;
bool DAG_node_is_full(uint voxel_count_flags) ;
uint EXTRACT_COUNT(uint a_leftOffset) ;
uint EXTRACT_START(uint a_leftOffset) ;
float3 mymul4x3(float4x4 m, float3 v) ;
bool first_hit_is_closest(uint tag) ;
bool isLeafAndIntersect(uint flags) ;
bool notLeafAndIntersect(uint flags) ;
bool isLeafOrNotIntersect(uint flags) ;
float3 decode_normal(float2 e) ;
uint SuperBlockIndex2DOpt(uint tidX, uint tidY, uint a_width) ;
float3 matmul4x3(float4x4 m, float3 v) ;
...
float3 matmul3x3(float4x4 m, float3 v) ;
...
uint get_links_end(in SCom2Header header, uint child_n, uint node1, uint node2) {
  if (header.has_multi_nodes != 0)
  {
    uint sh = child_n*header.reference_bits;
    return sh >= 32 ? ((node2 >> sh%32) & header.reference_mask) : ((node1 >> sh) & header.reference_mask);
  }
  else if (header.has_surfaces > 0)
  {
    uint base_links_end = ((node1 & 0xFFu) == SCOM2_CHILD_EMPTY ? 0 : 1);
    uint ch_active = (node1 >> header.children_active_bits_shift) & header.children_active_bits_mask;
    uint ch_n = bitCount(ch_active & ((1<<(1+child_n))-1));
...
uint EXTRACT_COUNT(uint a_leftOffset) { return (a_leftOffset & SIZE_MASK) >> 28; }
...
float3 decode_normal(float2 e) {
  ...
  if (v.z < 0) 
  {
    ...
    v.y = (1.0f - abs(vx)) * ((v.y >= 0.0f) ? +1.0f : -1.0f);
  }
  return normalize(v);
}
