...
bool BVHRT::need_normal()
{
  return m_preset.render_mode == MULTI_RENDER_MODE_LAMBERT_NO_TEX || 
         ...
         m_preset.render_mode == MULTI_RENDER_MODE_LOD ||
         m_preset.render_mode == MULTI_RENDER_MODE_GEOM ||
         m_preset.render_mode == MULTI_RENDER_MODE_PHONG ||
         m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL ||
         m_preset.render_mode == MULTI_RENDER_MODE_PBR ||
         m_preset.render_mode == MULTI_RENDER_MODE_PBR_NO_TEX ||
         m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL_LAMBERT;
}
...
uint32_t BVHRT::IntersectAllPrimitivesInLeaf(float4 rayPosAndNear, float4 rayDirAndFar, CRT_LeafInfo info, CRT_Hit* pHit)
{
  const float3 ray_pos = to_float3(rayPosAndNear);
  const float3 ray_dir = to_float3(rayDirAndFar);
  const float tNear    = rayPosAndNear.w;
  const float tPrev    = pHit->t;

  const uint32_t globalAABBId       = startEnd[info.geomId].x + info.aabbId;
  const uint32_t start_count_packed = m_primIdCount[globalAABBId];
  const uint32_t a_start = EXTRACT_START(start_count_packed);
  const uint32_t a_count = EXTRACT_COUNT(start_count_packed);

  const uint32_t geomTag = m_geomTags[info.geomId];
  switch(geomTag)
  {
    #ifndef DISABLE_MESH
    case TAG_TRIANGLE: 
    IntersectAllTrianglesInLeaf(ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif

    #ifndef DISABLE_SDF_GRID
    case TAG_SDF_GRID:
    IntersectSDFGrid(ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif
    
    #if !defined(DISABLE_SDF_SVS) || !defined(DISABLE_SDF_SBS) || !defined(DISABLE_SDF_FRAME_OCTREE)
    case TAG_SDF_NODE:
    OctreeNodeIntersect(m_geomData[info.geomId].type, ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif

    #if !defined(DISABLE_SDF_SVS) || !defined(DISABLE_SDF_FRAME_OCTREE_TEX) || !defined(DISABLE_SDF_FRAME_OCTREE)
    case TAG_SDF_BRICK:
    OctreeBrickIntersect(m_geomData[info.geomId].type, ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif
    
    #ifndef DISABLE_GRAPHICS_PRIM
    case TAG_GRAPHICS_PRIM:
    IntersectGraphicPrims(ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif
    
    #ifndef DISABLE_SDF_COCTREE_V3
    case TAG_COCTREE_BRICKED:
    COctreeIntersectV3(m_geomData[info.geomId].type, ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif
    
    #ifndef DISABLE_SDF_MULTI_OCTREE
    case TAG_SDF_OCTREE:
    OctreeIntersect(m_geomData[info.geomId].type, ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif

    #ifndef DISABLE_SDF_DAG
    case TAG_SDF_DAG:
    DAGIntersect(m_geomData[info.geomId].type, ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif

    #ifndef DISABLE_SCOM2
    case TAG_SCOM2:
    if (support_fast_scom_traversal)
      SCom2IntersectFast(m_geomData[info.geomId].type, ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    else
      SCom2Intersect(m_geomData[info.geomId].type, ray_pos, ray_dir, tNear, info.instId, info.geomId, a_start, a_count, pHit);
    break;
    #endif
  };

  if (m_preset.render_mode == MULTI_RENDER_MODE_INTERSECTION_COUNT)
    pHit->instId = (pHit->instId == 0xFFFFFFFF) ? 1 : pHit->instId+1; 

  return (pHit->t >= tPrev) ? TAG_NONE : geomTag;
}
                                ...
                                float &qNear, float &qFar, float2 &fNearFar, float3 &start_q)
{
  ...
  else if (type == TYPE_SDF_SVS)
  {
    ...
    for (int i=0;i<8;i++)
      values[i] = -d_max + 2.0f*d_max*(1.0f/255.0f)*((m_SdfSVSNodes[nodeId].values[i/4] >> (8*(i%4))) & 0xFF);
                                ...
                                CRT_Hit *pHit)
{
  ...
  else if (m_preset.sdf_node_intersect == SDF_OCTREE_NODE_INTERSECT_ST)
  {
    ...
    while (t < qFar && dist > EPS && iter < ST_max_iters)
    {
      ...
      point[0] = (start_q + t * ray_dir).x;
      point[1] = (start_q + t * ray_dir).y;
      point[2] = (start_q + t * ray_dir).z;
...
static bool is_compact_type(uint32_t type)
{
  return type == TYPE_SDF_DAG || type == TYPE_SCOM2;
}
...
void BVHRT::LocalSurfaceIntersection(uint32_t type, const float3 ray_dir, uint32_t instId, uint32_t geomId,
                                     ...
                                     CRT_Hit *pHit)
{
  const float BASE_EPS = 1e-6f;
  const float EPS = (is_surface_node && is_compact_type(type)) ? std::max(BASE_EPS, m_preset.compact_sdf_eps*d) : BASE_EPS;
  float d_inv = 1.0f / d;
  float t = qNear;
  ...
  */
  if (is_surface_node || m_preset.representation_mode == REPRESENTATION_MODE_SURFACE)
    start_sign = sign(start_dist);
  start_dist *= start_sign;
       ...
       //    m_preset.sdf_node_intersect == SDF_OCTREE_NODE_INTERSECT_IT)
  {
    ...
    else if (m_preset.sdf_node_intersect == SDF_OCTREE_NODE_INTERSECT_NEWTON)
    {
      ...
      if (nwt_min < nwt_max)
      {
        ...
        float f = 1000;
        while (nwt_iter < max_iters && std::abs(f) >= EPS)
        {
    ...
    else //if (m_preset.sdf_node_intersect == SDF_OCTREE_NODE_INTERSECT_IT)
    {
      ...
      float e = 0.1f*qFar;
      float t_max = std::abs(c3) < EPS ? 1e6f : -c2/(3*c3);
  ...
  if (debug_cur_pixel)
  {
    ...
    printf("t = %f in [%f, %f], tReal = %f in [%f %f]\n",t,qNear,qFar,tReal,fNearFar.x,fNearFar.y);
  ...
  if (t <= qFar && hit && tReal < pHit->t)
  {
    ...
    if (need_normal())
    {
      if (early_hit && qNear > 1e-6f) //hit cutting plane, return its normal
      {
        norm = -1.0f*m_cuttingPlane.normal;
      }
      else if (early_hit && m_preset.sdf_node_intersect == SDF_OCTREE_NODE_INTERSECT_BBOX) //hit side of the voxel's bbox, return its normal
      {
        norm = float3(0,0,0);
        float3 v = float3(std::max(start_q.x, 1 - start_q.x),
                          std::max(start_q.y, 1 - start_q.y),
                          std::max(start_q.z, 1 - start_q.z));
        int dim = 0;
        if (v.y > v.x && v.y > v.z)
          dim = 1;
        else if (v.z > v.x && v.z > v.y)
          dim = 2;
        norm[dim] = start_q[dim] < 0.5f ? -1.0f : 1.0f;
      }
      else //hit SDF surface, calculate normal with finite differences
      {
        float3 p0 = start_q + t * ray_dir;
        const float h = 0.001f;
        ...
        float ddz = (eval_dist_trilinear(values, p0 + float3(0, 0, h)) -
                    ...
                    (2 * h);
        norm = start_sign*float3(ddx, ddy, ddz);
      }
      norm = normalize(matmul4x3(m_instanceData[instId].transformInvTransposed, norm));
    }
    ...
    // store interpolation quotients instead of normal and tc
    if ((type == TYPE_SDF_MULTI_OCTREE || type == TYPE_SDF_DAG || type == TYPE_SCOM2) &&
        (m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL || m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL_LAMBERT))
    {
      dp = clamp(float(0xFFFFu)*dp, 0.0f, float(0xFFFFu));
      int3 dp_u16 = int3(dp);
      pHit->coords[0] = as_float((dp_u16.y << 16) | dp_u16.x);
      pHit->coords[1] = as_float(dp_u16.z);
    }
  }
}
                                ...
                                CRT_Hit *pHit)
{
  ...
  LocalSurfaceIntersection(type, ray_dir, instId, geomId, values, nodeId, primId, d, qNear, qFar, fNearFar, start_q, false,/*in */
                           pHit); /*out*/
}
    ...
    for (int x = 0; x < 4; x++)
    {
      for (int y = 0; y < 4; y++)
      {
        for (int z = 0; z < 4; z++)
        {
          ...
          values[16 * z + 4 * y + x] = -d_max + mult * ((m_SdfSBSData[v_off + vId / vals_per_int] >> (bits * (vId % vals_per_int))) & max_val);
          vmin = std::min(vmin, values[16 * z + 4 * y + x]);
        }
      }
    }
                                 ...
                                 CRT_Hit *pHit)
{
  ...
  while (brick_fNearFar.x < brick_fNearFar.y && pHit->t == old_t)
  {
    ...
    if (tNear < fNearFar.x && vmin <= 0.0f)    
    {
      ...
      if (m_preset.interpolation_mode == INTERPOLATION_MODE_TRILINEAR)
      {
        LocalSurfaceIntersection(type, ray_dir, instId, geomId, trilinear_values, nodeId, primId, d, 0.0f, qFar, fNearFar, start_q, false,/*in */
                               pHit); /*out*/
      }
      else
      {
          ...
          need_normal() && pHit->t != old_t)
      {
        ...
        int3 voxelPos0 = int3(dp.x < 0.5f ? int(voxelPos.x) - 1 : int(voxelPos.x), 
                              dp.y < 0.5f ? int(voxelPos.y) - 1 : int(voxelPos.y), 
                              dp.z < 0.5f ? int(voxelPos.z) - 1 : int(voxelPos.z));
        for (int i=0;i<8;i++)
        {
  ...
  if (pHit->t < old_t)
  {
             ...
             header.aux_data == SDF_SBS_NODE_LAYOUT_ID32F_IRGB32F_IN)
    {
      ...
      const uint32_t baseOffs0 = m_SdfSBSData[t_off+0];
      const uint32_t baseOffs1 = m_SdfSBSData[t_off+1];
      const uint32_t baseOffs2 = m_SdfSBSData[t_off+2];
      const uint32_t baseOffs3 = m_SdfSBSData[t_off+3];
      const uint32_t baseOffs4 = m_SdfSBSData[t_off+4];
      const uint32_t baseOffs5 = m_SdfSBSData[t_off+5];
      const uint32_t baseOffs6 = m_SdfSBSData[t_off+6];
      const uint32_t baseOffs7 = m_SdfSBSData[t_off+7];

      float3 color = (1-dp.x)*(1-dp.y)*(1-dp.z)*float3(m_SdfSBSDataF[baseOffs0+0], m_SdfSBSDataF[baseOffs0+1], m_SdfSBSDataF[baseOffs0+2]) + 
                     (1-dp.x)*(1-dp.y)*(  dp.z)*float3(m_SdfSBSDataF[baseOffs1+0], m_SdfSBSDataF[baseOffs1+1], m_SdfSBSDataF[baseOffs1+2]) + 
                     (1-dp.x)*(  dp.y)*(1-dp.z)*float3(m_SdfSBSDataF[baseOffs2+0], m_SdfSBSDataF[baseOffs2+1], m_SdfSBSDataF[baseOffs2+2]) + 
                     (1-dp.x)*(  dp.y)*(  dp.z)*float3(m_SdfSBSDataF[baseOffs3+0], m_SdfSBSDataF[baseOffs3+1], m_SdfSBSDataF[baseOffs3+2]) + 
                     (  dp.x)*(1-dp.y)*(1-dp.z)*float3(m_SdfSBSDataF[baseOffs4+0], m_SdfSBSDataF[baseOffs4+1], m_SdfSBSDataF[baseOffs4+2]) + 
                     (  dp.x)*(1-dp.y)*(  dp.z)*float3(m_SdfSBSDataF[baseOffs5+0], m_SdfSBSDataF[baseOffs5+1], m_SdfSBSDataF[baseOffs5+2]) + 
                     (  dp.x)*(  dp.y)*(1-dp.z)*float3(m_SdfSBSDataF[baseOffs6+0], m_SdfSBSDataF[baseOffs6+1], m_SdfSBSDataF[baseOffs6+2]) + 
                     (  dp.x)*(  dp.y)*(  dp.z)*float3(m_SdfSBSDataF[baseOffs7+0], m_SdfSBSDataF[baseOffs7+1], m_SdfSBSDataF[baseOffs7+2]);
...
void BVHRT::IntersectSDFGrid(const float3 ray_pos, const float3 ray_dir,
                             float tNear, uint32_t instId, uint32_t geomId,
                             uint32_t a_start, uint32_t a_count,
                             CRT_Hit *pHit)
{
#ifndef DISABLE_SDF_GRID
  const uint32_t type = m_geomData[geomId].type;
  const uint32_t sdfId = m_geomData[geomId].offset.x;
  const uint32_t primId = 0;
  const float3 min_pos = float3(-1, -1, -1);
  const float3 max_pos = float3(1, 1, 1);
  const float dir_len = length(ray_dir);
  const float3 dir = ray_dir / dir_len;

  const float EPS = 1e-5f;
  float2 tNear_tFar = box_intersects(min_pos, max_pos, ray_pos, dir);
  ...
  uint32_t iter = 0;
  float d = eval_distance_sdf_grid(sdfId, ray_pos + t * dir);
  while (iter < 1000 && d > EPS && t < tFar)
  {
    t += d + EPS;
    d = eval_distance_sdf_grid(sdfId, ray_pos + t * dir);
    iter++;
  }
  ...
  if (t > tNear && t < pHit->t)
  {
    ...
    {
      const float h = 0.001f;
      float ddx = (eval_distance_sdf_grid(sdfId, p0 + float3(h, 0, 0)) -
                   eval_distance_sdf_grid(sdfId, p0 + float3(-h, 0, 0))) /
                  (2 * h);
      float ddy = (eval_distance_sdf_grid(sdfId, p0 + float3(0, h, 0)) -
                   eval_distance_sdf_grid(sdfId, p0 + float3(0, -h, 0))) /
                  (2 * h);
      float ddz = (eval_distance_sdf_grid(sdfId, p0 + float3(0, 0, h)) -
                   eval_distance_sdf_grid(sdfId, p0 + float3(0, 0, -h))) /
                  (2 * h);

      norm = normalize(float3(ddx, ddy, ddz));
    }
    float3 n = normalize(matmul4x3(m_instanceData[instId].transformInvTransposed, norm));
                                  ...
                                  uint32_t a_count, CRT_Hit* pHit)
{
  ...
  GraphicsPrimHeader header = m_GraphicsPrimHeaders[m_geomData[geomId].offset.x];
  ...
  uint32_t start_index = m_geomData[geomId].offset.y + prim_len * a_start;
...
float BVHRT::eval_distance_sdf_grid(uint32_t grid_id, float3 pos)
{
  ...
  float3 grid_size_f = float3(size - 1);
  ...
  else if (m_preset.interpolation_mode == INTERPOLATION_MODE_TRICUBIC)
  {
    ...
    {
      uint32_t x0 = vox_u.x - 1u;
      uint32_t y0 = vox_u.y - 1u;
      uint32_t z0 = vox_u.z - 1u;
                                                  ...
                                                  float values[8] /*out*/)
{
  ...
  float add_transform = header.sim_compression > 0 ? 2.0f*1.73205081f*sz_inv*(2.0f*(float(transform_code & header.add_mask) / float(header.add_mask)) - 1.0f) : 0.0f;
  ...
  uint32_t vId0 = uint32_t(dot(m_SdfCompactOctreeRotModifiers[rotIdx], 
                           int4(int(voxelPos.x) + int(header.brick_pad), int(voxelPos.y) + int(header.brick_pad), int(voxelPos.z) + int(header.brick_pad), 1)));
  uint32_t vId, dist;
                                                     ...
                                                     float values[8] /*out*/)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    uint32_t localId = uint32_t(dot(m_SdfCompactOctreeRotModifiers[rotIdx], int4((int)vPosOrig.x, (int)vPosOrig.y, (int)vPosOrig.z, 1)));
                                                    ...
                                                    float values[8] /*out*/)
{
  ...
  uint32_t p_size = header.brick_size + 2 * header.brick_pad;
  uint32_t PFlagPos = uint32_t(dot(m_SdfCompactOctreeRotModifiers[ROT_COUNT + rotIdx], 
                               int4((int)(voxelPos.x + header.brick_pad), (int)(voxelPos.y + header.brick_pad), (int)(voxelPos.z + header.brick_pad), 1)));
  ...
  {
    float3 vPosOrig = voxelPos + float3(header.brick_pad) + float3((i & 4) >> 2, (i & 2) >> 1, i & 1);
    uint32_t voxelId = uint32_t(dot(m_SdfCompactOctreeRotModifiers[rotIdx], int4((int)vPosOrig.x, (int)vPosOrig.y, (int)vPosOrig.z, 1)));
...
void BVHRT::COctreeV3_BrickIntersect(uint32_t type, const float3 ray_pos, const float3 ray_dir,
                                     float tNear, uint32_t instId, uint32_t geomId, // const COctreeV3Header &header, --> coctree_v3_header
                                     ...
                                     CRT_Hit *pHit)
{
  ...
  uint32_t v_size = coctree_v3_header.brick_size + 2*coctree_v3_header.brick_pad + 1;
  ...
  d = 2.0f/(sz*coctree_v3_header.brick_size);
  ...
  {
    float3 hit_pos = ray_pos + brick_fNearFar.x*ray_dir;
    float3 local_pos = (hit_pos - brick_min_pos) * (0.5f*sz*coctree_v3_header.brick_size);
    float3 voxelPos = floor(clamp(local_pos, 1e-6f, coctree_v3_header.brick_size-1e-6f));

    float3 min_pos = brick_min_pos + d*voxelPos;
    float3 max_pos = min_pos + d*float3(1,1,1);
    float3 size = max_pos - min_pos;
    
    auto header = coctree_v3_header;
    ...
    {
      float3 start_pos = ray_pos + fNearFar.x*ray_dir;
      start_q = (start_pos - min_pos) * (0.5f*sz*coctree_v3_header.brick_size);
      qFar = (fNearFar.y - fNearFar.x) * (0.5f*sz*coctree_v3_header.brick_size);
    
      LocalSurfaceIntersection(type, ray_dir, instId, geomId, values, brickOffset, brickOffset, d, 0.0f, qFar, fNearFar, start_q, false,/*in */
                               pHit); /*out*/
      ...
      if (m_preset.normal_mode == NORMAL_MODE_SDF_SMOOTHED && need_normal() && pHit->t < old_t)
      {
        ...
        int3 voxelPos0 = int3((int)(dp.x < 0.5f ? voxelPos.x - 1 : voxelPos.x), 
                              (int)(dp.y < 0.5f ? voxelPos.y - 1 : voxelPos.y), 
                              (int)(dp.z < 0.5f ? voxelPos.z - 1 : voxelPos.z));
        for (int i=0;i<8;i++)
        {
          int3 VoxelPosI = voxelPos0 + int3((i & 4) >> 2, (i & 2) >> 1, i & 1);
          float3 dVoxelPos = float3(VoxelPosI) - voxelPos;
  ...
  //ray hit a brick
  if (pHit->t < old_t && coctree_v3_header.uv_size > 0)
  {
    uint32_t p_size = coctree_v3_header.brick_size + 2 * coctree_v3_header.brick_pad;
    ...
    unsigned off_3 = off_2 + distance_offsets_size_uints;   // min value and range
    unsigned off_4 = off_3 + min_range_size_uints;          // texture coordinates
    unsigned off_5 = off_4 + 8 * coctree_v3_header.uv_size; // distances
...
void BVHRT::COctreeIntersectV3(uint32_t model_type, const float3 ray_pos, const float3 ray_dir,
                               float tNear, uint32_t instId, uint32_t geomId,
                               uint32_t bvhNodeId, uint32_t a_count,
                               CRT_Hit *pHit)
{
  ...
  stack[top].p_size = uint2((start_p.x << 16) | start_p.y, (start_p.z << 16) | start_sz);
    ...
    while (top >= 0)
    {
      ...
      if (is_leaf)
      {
        ...
        COctreeV3_BrickIntersect(TYPE_SDF_FRAME_OCTREE, ray_pos, ray_dir, tNear, instId, geomId, leafOffset, 
                            ...
                            CRT_Hit *pHit)
{
  ...
  stack[top].nodeId = m_geomData[geomId].offset.x;
  stack[top].info = 0;
  ...
  while (top >= 0)
  {
    ...
    if (!is_leaf)
    {
      ...
      {
        stack[top].info = m_SdfMultiOctreeNodes[stack[top].nodeId].data_offset;
        is_leaf = true;

        // we should stop on this LOD level, but this node is empty, so just skip it
        if (m_SdfMultiOctreeNodes[stack[top].nodeId].voxel_count == 0)
        {
          top--;
          continue; //
        }
    ...
    if (is_leaf)
    {
...

      {
        ...
        qNear = std::max(0.0f, sz * (tNear - fNearFar.x));
      ...
      if (debug_cur_pixel)
      {
        printf("qNear, qFar = [%f %f]\n", qNear, qFar);
      }
#endif

      for (int voxel_id = 0; voxel_id < m_SdfMultiOctreeNodes[stack[top].nodeId].voxel_count; voxel_id++)
      {
        for (int i = 0; i < 8; i++)
          values[i] = m_SdfMultiOctreeValues[stack[top].info + 8*voxel_id + i];
        
        bool is_surface_node = (m_SdfMultiOctreeNodes[stack[top].nodeId].flags & MULTI_OCTREE_FLAG_SURFACE) > 0;
        uint32_t primId = nodeId - m_geomData[geomId].offset.x; //local primitive id in this geometry
        LocalSurfaceIntersection(TYPE_SDF_MULTI_OCTREE, ray_dir, instId, geomId, values, nodeId, primId, d, qNear, qFar, fNearFar, 
                                 start_q, is_surface_node, pHit);                                                                                          
      }

      if (m_preset.render_mode == MULTI_RENDER_MODE_MO_VOXEL_COUNT)
      {
        if (pHit->t < old_t)
          pHit->primId = m_SdfMultiOctreeNodes[stack[top].nodeId].voxel_count;
      }
      else if (m_preset.render_mode == MULTI_RENDER_MODE_MO_FLAGS)
      {
        if (pHit->t < old_t)
          pHit->primId = m_SdfMultiOctreeNodes[stack[top].nodeId].flags;
      }
    ...
    else
    {
      ...
      do
      {
        uint32_t childrenOff = m_SdfMultiOctreeNodes[stack[top].nodeId].children_offset;
        uint32_t childNode = currNode ^ a; // child node number, from 0 to 8
        uint32_t childVoxelCount = m_SdfMultiOctreeNodes[childrenOff + childNode].voxel_count;
        bool childIsNode = m_SdfMultiOctreeNodes[childrenOff + childNode].children_offset > 0;

        // if child is active
        if (childVoxelCount > 0 || childIsNode)
        {
          tmp_buf[buf_top].nodeId = childrenOff + childNode;
          tmp_buf[buf_top].info = childIsNode ? 0 : m_SdfMultiOctreeNodes[childrenOff + childNode].data_offset; // > 0 is child is leaf
          tmp_buf[buf_top].p_size = (stack[top].p_size << 1) | uint2(((currNode & 4) << (16 - 2)) | ((currNode & 2) >> 1), (currNode & 1) << 16);
          buf_top++;
        }
        currNode = new_node(((currNode & 4) > 0) ? t1.x : tm.x, nn_indices[currNode].x,
...
void BVHRT::DAGIntersect(uint32_t type, const float3 ray_pos, const float3 ray_dir,
                         float tNear, uint32_t instId, uint32_t geomId,
                         uint32_t a_start, uint32_t a_count,
                         CRT_Hit *pHit)
{
  ...
  uint32_t nodeId;
  uint32_t level_sz;
  float d, level_sz_f, sz_inv, qNear, qFar;
  float2 fNearFar;
  float3 start_q;
  float values[8];
  float old_t = pHit->t;
  SdfDAGHeader header = m_SdfDAGHeaders[m_geomData[geomId].offset.y];
  ...
  if (debug_cur_pixel)
  {
    ...
    if (is_leaf)
    {
      ...
      if (DAG_node_is_full(node.voxel_count_flags) && node.data_edges_offset > 0)
      {
        ...
        while (brick_fNearFar.x < brick_fNearFar.y && pHit->t == old_t)
        {
          ...
          uint32_t s_id = 0;
          while (s_id < surface_count && pHit->t == old_t)
          {
            ...
            uint32_t rotIdx = m_RotAddTable[curElem.info*m_SdfDAGTransformCount + 
            ...
            for (int i=0;i<8;i++)
            {
              ...
              uint32_t vId0 = m_SdfDAGTranspositions[rotIdx*v_size*v_size*v_size + idx];
              float val = m_SdfDAGDistances[offset + vId0] + add;
              values[i] = val - m_preset.compact_sdf_eps*d;
            ...
            if (fNearFar.x < fNearFar.y && vmin <= 0.0f && vmax >= 0.0f)    
            {
              ...
              start_q = vox_sz * (start_pos - min_pos);
              qNear = std::max(0.0f, vox_sz * (tNear - fNearFar.x));
              qFar = vox_sz * (fNearFar.y - fNearFar.x);
          
              LocalSurfaceIntersection(type, ray_dir, instId, geomId, values, curElem.nodeId, primId, d, qNear, qFar, fNearFar, start_q, is_surface,/*in */
                                       pHit); /*out*/
            }

            // if we need channel data, we must use rotations, but we cannot do it in resolve pass (no rotation info there)
            // however, we can transform interpolation quotients here that will lead to the same result
            if (pHit->t != old_t && 
               (m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL || m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL_LAMBERT))
            {
              uint32_t channelRotId = m_RotAddTable[curElem.info*m_SdfDAGTransformCount + 
                                                    node.channels_edge.rotation_id];
              float t = (pHit->t - fNearFar.x) / d;
              float3 dp = start_q + t * ray_dir;
              float3 dp_rot = to_float3(m_SdfCompactOctreeRotTransforms[channelRotId] * to_float4(dp, 1.0f));
              dp_rot = clamp(float(0xFFFFu)*dp_rot, 0.0f, float(0xFFFFu));
              int3 dp_u16 = int3(dp_rot);
              pHit->coords[0] = as_float((dp_u16.y << 16) | dp_u16.x);
              pHit->coords[1] = as_float(dp_u16.z);
            }
    ...
    else if (brick_fNearFar.x < brick_fNearFar.y)
    {
      ...
      while (neg_brick_fNearFar.x < neg_brick_fNearFar.y)
      {
        ...
        int idx = int(voxelPos.x)*v_sz*v_sz + int(voxelPos.y)*v_sz + int(voxelPos.z);
        uint32_t voxId = m_SdfDAGTranspositions[m_SdfDAGBrickTranspositionOffset + curElem.info*v_sz*v_sz*v_sz + idx];
        ...
        if (tNear < fNearFar.x && edge.child_offset > 0)    
        {
          ...
          stack[top].info = m_RotAddTable[curElem.info*m_SdfDAGTransformCount + edge.rotation_id];
...
void BVHRT::SCom2Intersect(uint32_t type, const float3 ray_pos, const float3 ray_dir,
                           float tNear, uint32_t instId, uint32_t geomId,
                           uint32_t a_start, uint32_t a_count,
                           CRT_Hit *pHit)
{
  ...
  uint32_t level_sz;
  float d, level_sz_f;
  SCom2Header header = m_SCom2Headers[m_geomData[geomId].offset.y];

  stack[top].nodeId = header.nodes_arr_offset + header.root_node_off;
  stack[top].info = 0; // info here is cumulative rotation id of the node ( 8 bits ),
                       // number of surfaces if it is leaf ( 16 bits ),
                       // and if it is surface node or not if it is leaf ( 1 bit )
  stack[top].p_size = uint2(0, 1);
  ...
  while (top >= 0)
  {
    ...
    if (std::min(t1.x, std::min(t1.y, t1.z)) < tNear)
      continue;

#ifdef ON_CPU
    if (debug_cur_pixel)
    {
      printf("[%d %d %d %d] node %d rot %d\n", p.x, p.y, p.z, level_sz, curElem.nodeId, curElem.info & 0xFF);
    }
#endif
    bool is_leaf = curElem.info > 0xFF;
    bool is_surface = curElem.info > 0xFFFFFFu;
    uint32_t surface_count = (curElem.info >> 8) & 0xFFFF;
    if (!is_leaf)
    {
      float t = m_preset.fixed_lod > 0 ? 1.0f : std::max(t0.x, std::max(t0.y, t0.z));
      float target_lod_size = (std::pow(2.0f, m_preset.level_of_detail) - 0.01f) / t;
      is_leaf = float(level_sz) > target_lod_size;
      if (is_leaf) // we need to switch to leaf node for LOD of this node
      {
        NodeHeadUnpacked node = unpack_node_head(header, m_SCom2Data[curElem.nodeId], m_SCom2Data[curElem.nodeId + header.links_offset - 1]);
        if (node.base_type == SCOM2_CHILD_EMPTY)
          continue;
        is_surface = (node.base_type & SCOM2_CHILD_TYPE_MASK) == SCOM2_CHILD_LEAF_SURFACE;
        curElem.nodeId = curElem.nodeId + header.links_offset;
        surface_count = node.base_links_end - header.has_channels;
      }
    }
    ...
    {
      // intersect with grid inside leaf
      const float sz_inv = 2.0f / level_sz_f;
      const uint32_t p_mask = level_sz - 1;
      const uint3 real_p = uint3(((a & 4) > 0) ? (~p.x & p_mask) : p.x,
                                 ((a & 2) > 0) ? (~p.y & p_mask) : p.y,
                                 ((a & 1) > 0) ? (~p.z & p_mask) : p.z);
      float3 brick_min_pos = float3(-1, -1, -1) + sz_inv * float3(real_p);
      float tmin = std::max(t0.x, std::max(t0.y, t0.z));
      float tmax = std::min(t1.x, std::min(t1.y, t1.z));
      uint32_t rotId = curElem.info & 0xFF;
      float max_val = get_max_sdf_val(level_sz_f);
      d = 2.0f / (level_sz_f * header.brick_size);
      const float old_t = pHit->t;

      while (tmin < tmax && pHit->t == old_t)
      {
        uint32_t v_size = header.brick_size + 1;
        float3 hit_pos = ray_pos + tmin * ray_dir;
        float3 local_pos = (hit_pos - brick_min_pos) * (0.5f * level_sz_f * header.brick_size);
        float3 voxelPos = floor(clamp(local_pos, 1e-6f, header.brick_size - 1e-6f));
        ...
        while (fNearFar.x < fNearFar.y && s_id < surface_count && pHit->t == old_t)
        {
          uint32_t link_0 = curElem.nodeId + s_id * header.uints_per_link;
          uint32_t link_1 = link_0 + header.child_offset_off;
          SdfDAGDataEdge de = unpack_data_edge(header, max_val, m_SCom2Data[link_0], m_SCom2Data[link_1]);
          uint32_t offset = header.bricks_arr_offset + header.bricks_step * de.data_offset;
          int4 voxelPosI = int4(int(voxelPos.x), int(voxelPos.y), int(voxelPos.z), 1);
          uint32_t rotIdx = m_RotAddTable[rotId * ROT_COUNT + de.rotation_id];
          float add = de.add;
          float vmin = 1.0f;
          float vmax = is_surface ? -1.0f : 1.0f;
          float3 start_q;
          ...
          for (int i = 0; i < 8; i++)
          {
            ...
            uint32_t p_val = m_SCom2Data[offset + vId0 / header.values_per_uint];
            uint32_t p_off = (vId0 % header.values_per_uint) * header.bits_per_value;
          ...
          if (vmin <= 0.0f && vmax >= 0.0f)
          {
            ...
            start_q = vox_sz * (start_pos - min_pos);
            const float qNear = std::max(0.0f, vox_sz * (tNear - fNearFar.x));
            const float qFar = vox_sz * (fNearFar.y - fNearFar.x);

            LocalSurfaceIntersection(type, ray_dir, instId, geomId, values, curElem.nodeId, curElem.nodeId, d, qNear, qFar, fNearFar, start_q, is_surface, /*in */
                                     pHit);                                                                                                                /*out*/
          }
          ...
          // if we need channel data, we must use rotations, but we cannot do it in resolve pass (no rotation info there)
          // however, we can transform interpolation quotients here that will lead to the same result
          if (pHit->t != old_t && header.has_channels > 0 && m_preset.render_mode != MULTI_RENDER_MODE_PRIMITIVE)
          {
            uint32_t ch_off = curElem.nodeId + surface_count * header.uints_per_link;
            SdfDAGChildEdge channel_edge = unpack_child_edge(header, m_SCom2Data[ch_off], m_SCom2Data[ch_off + header.child_offset_off]);
            pHit->primId = channel_edge.child_offset;
            if (m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL || m_preset.render_mode == MULTI_RENDER_MODE_CHANNEL_LAMBERT)
            {
              uint32_t channelRotId = m_RotAddTable[(curElem.info & 0xFF) * ROT_COUNT + channel_edge.rotation_id];
              float t = (pHit->t - fNearFar.x) / d;
              float3 dp = start_q + t * ray_dir;
              float3 dp_rot = to_float3(m_SdfCompactOctreeRotTransforms[channelRotId] * to_float4(dp, 1.0f));
              dp_rot = clamp(float(0xFFFFu)*dp_rot, 0.0f, float(0xFFFFu));
        ...
        tmin += std::max(0.0f, fNearFar.y - tmin) + 1e-5f;

        if (pHit->t < old_t)
        {
          top = -1;
          if (m_preset.render_mode == MULTI_RENDER_MODE_LOD)
            pHit->primId = uint32_t(std::log2(float(level_sz*header.brick_size)));
        }
      }
    }
    else
    {
      NodeHeadUnpacked node = unpack_node_head(header, m_SCom2Data[curElem.nodeId], m_SCom2Data[curElem.nodeId + header.links_offset - 1]);
      uint32_t node1 = m_SCom2Data[curElem.nodeId + header.references_offset];
      uint32_t node2 = m_SCom2Data[curElem.nodeId + header.links_offset - 1];

      int buf_top = 0;
      tm = 0.5f * (t0 + t1);

      int currNode = first_node(t0, tm);
      do
      {
        uint32_t rawChildNode = currNode ^ a; // child node number, from 0 to 8
        ...
        uint32_t ch_type = (node.children_types >> SCOM2_CHILD_TYPE_BITS * voxId) & SCOM2_CHILD_TYPE_MASK;
        uint32_t ch_start_link = voxId == 0 ? node.base_links_end : get_links_end(header, voxId - 1, node1, node2);
        uint32_t ch_end_link = get_links_end(header, voxId, node1, node2);

        // if we itersected with this node bbox and this node is valid
        if (ch_type != SCOM2_CHILD_EMPTY)
        {
          tmp_buf[buf_top].p_size = (curElem.p_size << 1) | uint2(((currNode & 4) << (16 - 2)) | ((currNode & 2) >> 1), (currNode & 1) << 16);

          uint32_t link_off = curElem.nodeId + header.links_offset + ch_start_link * header.uints_per_link;
          if (ch_type == SCOM2_CHILD_NODE)
          {
            SdfDAGChildEdge ce = unpack_child_edge(header, m_SCom2Data[link_off],
                                                   m_SCom2Data[link_off + header.child_offset_off]);
            tmp_buf[buf_top].nodeId = header.nodes_arr_offset + ce.child_offset;
            tmp_buf[buf_top].info = m_RotAddTable[curElem.info * ROT_COUNT + ce.rotation_id];
          }
          else // SCOM2_CHILD_LEAF_VOLUME or SCOM2_CHILD_LEAF_SURFACE
          {
            uint32_t surf_count = ch_end_link - ch_start_link - header.has_channels;
            tmp_buf[buf_top].nodeId = link_off;
            tmp_buf[buf_top].info = (curElem.info & 0xFF) | (surf_count << 8) | (ch_type == SCOM2_CHILD_LEAF_SURFACE ? (1 << 24) : 0);
          }
          buf_top++;
        }
      ...
      for (int i = 1; i <= buf_top; i++)
      {
        stack[top + i] = tmp_buf[buf_top - i];
      }
      top += buf_top;
    }
  }
...
void BVHRT::LocalSurfaceIntersectionFast(uint32_t type, const float3 ray_dir, uint32_t instId, uint32_t geomId,
                                         float values[8], uint32_t nodeId, uint32_t primId, float d, float qNear, 
                                         float qFar, float2 fNearFar, float3 start_q, bool is_surface_node,
                                         CRT_Hit *pHit)
{
...
void BVHRT::SCom2IntersectFast(uint32_t type, const float3 ray_pos, const float3 ray_dir,
                               float tNear, uint32_t instId, uint32_t geomId,
                               uint32_t a_start, uint32_t a_count,
                               CRT_Hit *pHit)
{
  ...
  SCom2Header header = m_SCom2Headers[m_geomData[geomId].offset.y];
  {
    uint32_t nodeData0 = m_SCom2Data[header.root_node_off];
    NodeHeadUnpacked node = unpack_node_head(header, nodeData0, nodeData0);
    uint32_t currNode = first_node(t0, tm);

    cur.linksOff = header.root_node_off + header.links_offset + ((node.base_type) == SCOM2_CHILD_EMPTY ? 0 : 1);
    cur.info = (0 << 24) | (node.children_types << 8) | currNode;
    cur.transform = 0;
  ...
  while (top >= 0)
  {
    ...
    uint32_t childN = uint32_t(dot(m_SdfCompactOctreeRotModifiers[2 * ROT_COUNT + cur.transform], // child node number, from 0 to 8
                                   int4((int)voxelPos.x, (int)voxelPos.y, (int)voxelPos.z, 1)));
    uint32_t activeChildrenMask = (((cur.info & 0x00AAAA00u) >> 1) | (cur.info & 0x00555500u)) >> 8;
    uint32_t childLink = cur.linksOff + bitCount(activeChildrenMask & ((1u << (2 * childN)) - 1));
    uint32_t childHasData = activeChildrenMask & (1u << (2 * childN));
    uint32_t childIsLeaf = cur.info & (1u << (8 + 2 * childN));
    uint32_t rotId = cur.info >> 24;
    ...
    if (std::min(std::min(tl.x, tl.y), tl.z) < tNear || childHasData == 0)
    {
    ...
    else if (childIsLeaf > 0) // we intersected with leaf, fill the hit data and return
    {
      ...
      float3 p_f = float3(p);
      t0 = _t0 + 0.5f * d * p_f * _l;
      float3 t1 = _t0 + 0.5f * d * (p_f + 1) * _l;

      float max_val = get_max_sdf_val(float(level_sz));

      bool childIsSurface = (cur.info & (1u << (8 + 2 * childN + 1))) > 0;
      uint32_t linkData0 = m_SCom2Data[childLink];
      SdfDAGDataEdge de = unpack_data_edge(header, max_val, linkData0, linkData0);
      uint32_t offset = header.bricks_arr_offset + header.bricks_step * de.data_offset;
      uint32_t rotIdx = m_RotAddTable[rotId * ROT_COUNT + de.rotation_id];
      float add = de.add;

      float3 vox_tm = 0.5f * (t0 + t1);
      uint32_t vox_currNode = first_node(t0, vox_tm);

      while (vox_currNode < 8)
      {
        float3 vox_tl = float3((vox_currNode & 4) > 0 ? t1.x : vox_tm.x,
                               (vox_currNode & 2) > 0 ? t1.y : vox_tm.y,
                               (vox_currNode & 1) > 0 ? t1.z : vox_tm.z);

        uint32_t voxId = vox_currNode ^ a;

        float vmin = 1.0f;
        float vmax = childIsSurface ? -1.0f : 1.0f;
        float values[8];
        for (int i = 0; i < 8; i++)
        {
          int4 pI = int4((voxId & 4) >> 2, (voxId & 2) >> 1, voxId & 1, 0) + int4((i & 4) >> 2, (i & 2) >> 1, i & 1, 1);
          uint32_t vId0 = uint32_t(dot(m_SdfCompactOctreeRotModifiers[rotIdx], pI));
          uint32_t p_val = m_SCom2Data[offset + vId0 / header.values_per_uint];
          uint32_t p_off = (vId0 % header.values_per_uint) * header.bits_per_value;
          float val = max_val * (2.0f * ((p_val >> p_off) & header.value_mask) / float(header.value_mask) - 1) + add;
        ...
        if (vmin <= 0.0f && vmax >= 0.0f)
        {
          ...
          float3 vox_t0 = float3((vox_currNode & 4) > 0 ? vox_tm.x : t0.x,
                                 (vox_currNode & 2) > 0 ? vox_tm.y : t0.y,
                                 (vox_currNode & 1) > 0 ? vox_tm.z : t0.z);
          float3 vox_t1 = float3((vox_currNode & 4) > 0 ? t1.x : vox_tm.x,
                                 (vox_currNode & 2) > 0 ? t1.y : vox_tm.y,
                                 (vox_currNode & 1) > 0 ? t1.z : vox_tm.z);

          float tmin = std::max(vox_t0.x, std::max(vox_t0.y, vox_t0.z));
          float tmax = std::min(vox_t1.x, std::min(vox_t1.y, vox_t1.z));
          float3 start_pos = ray_pos + tmin * ray_dir;
          float3 min_pos = float3(-1, -1, -1) + d * float3(real_p.x, real_p.y, real_p.z) + 0.5f*d*float3((voxId & 4)>>2, (voxId & 2)>>1, voxId & 1);

          float vox_sz = float(level_sz);
          float3 start_q = vox_sz * (start_pos - min_pos);
          const float qNear = std::max(0.0f, vox_sz * (tNear - tmin));
          const float qFar = vox_sz * (tmax - tmin);

          //potentially faster, but somehow consumes more registers...
          /*
          const float d_inv = 1.0f / d;
          float t = qNear;
          float dist = eval_dist_trilinear(values, start_q + t * ray_dir);
          float3 pp0 = start_q + t * ray_dir;

          uint32_t iter = 0;
          while (t < qFar && dist > 1e-6f && iter < 256)
          {
            t += dist * d_inv;
          ...
          {
            float3 p0 = start_q + t * ray_dir;
            float3 norm = normalize(eval_dist_trilinear_diff(values, p0));
            float2 encoded_norm = encode_normal(norm);

            pHit->t = tmin + d * t;
            pHit->primId = childLink;
            pHit->instId = instId;
            pHit->geomId = geomId | (type << SH_TYPE);
            pHit->coords[0] = 0;
            pHit->coords[1] = 0;
          ...
          */
          ...
          float last_t = pHit->t;
          LocalSurfaceIntersectionFast(type, ray_dir, instId, geomId, values, childLink, childLink, 0.5f*d, qNear, qFar, float2(tmin, tmax), start_q, childIsSurface, /*in */
                                       pHit);                                                                                                                    /*out*/
          if (pHit->t < last_t)
            return;
        }

        vox_currNode = new_node(vox_currNode, vox_tl.x, vox_tl.y, vox_tl.z);
      }
    ...
    else // this child is a node, move to it
    {
      ...
      uint32_t linkData0 = m_SCom2Data[childLink];
      SdfDAGChildEdge ce = unpack_child_edge(header, linkData0, linkData0);
      uint32_t nodeData0 = m_SCom2Data[ce.child_offset];
      NodeHeadUnpacked node = unpack_node_head(header, nodeData0, nodeData0);
      uint32_t nextNode = first_node(t0, tm);
      uint32_t rotIdx = m_RotAddTable[rotId * ROT_COUNT + ce.rotation_id];

      cur.linksOff = ce.child_offset + header.links_offset + (node.base_type == SCOM2_CHILD_EMPTY ? 0 : 1);
      cur.transform = m_RotAddTable[cur.transform * ROT_COUNT + ce.rotation_id];
      cur.info = (rotIdx << 24) | (node.children_types << 8) | nextNode;
                                ...
                                CRT_Hit* pHit)
{
  ...
  while (top >= 0 && !hitFound)
  {
    ...
    {
#ifdef STOP_ON_BLAS_LEAF
      pHit->t = tNear;
      pHit->primId = EXTRACT_START(leftNodeOffset);
      pHit->instId = instId;
      pHit->geomId = geomId;
      hitFound = true;
#else
      CRT_LeafInfo leafInfo;
      leafInfo.primId = 0xFFFFFFFF;
      leafInfo.aabbId = EXTRACT_START(leftNodeOffset);
      leafInfo.instId = instId;
      leafInfo.geomId = geomId;
      leafInfo.rayxId = 0xFFFFFFFF;
      leafInfo.rayyId = 0xFFFFFFFF;

      const float SDF_BIAS = 0.01f;
      const float tNearSdf = std::max(tNear, SDF_BIAS);
      const uint32_t hitTag = IntersectAllPrimitivesInLeaf(to_float4(ray_pos, tNearSdf), to_float4(ray_dir, 1e9f), leafInfo, pHit);
...
CRT_Hit BVHRT::RayQuery_NearestHit(float4 posAndNear, float4 dirAndFar)
{
  ...
  SCom2IntersectFast(TYPE_SCOM2, to_float3(posAndNear), to_float3(dirAndFar), posAndNear.w, 0, 0, 0, 0, &hit);
  return hit;
#else
  { 
    uint32_t nodeIdx = 0;
...
CRT_Hit BVHRT::RayQuery_NearestHitWithRayInfo(float4 posAndNear, float4 dirAndFar)
{
  bool stopOnFirstHit = (dirAndFar.w <= 0.0f);
  if(stopOnFirstHit)
    dirAndFar.w *= -1.0f;
  ...
  RI.pos = to_float3(posAndNear);
  RI.dir = to_float3(dirAndFar);

  RI.intersected_prims_count = 0;
  RI.intersected_AABB_count = 0;

  RI.bboxes_info.clear();
  ...
  { 
    ...
    do
    {
      ...
      if(isLeafAndIntersect(travFlags)) 
      {
        ...
        BVH2TraverseF32WithRayInfo(ray_pos, ray_dir, posAndNear.w, instId, geomId, stopOnFirstHit, &hit);
      }
    } while (nodeIdx < 0xFFFFFFFE && !(stopOnFirstHit && hit.primId != uint32_t(-1))); //
  }
...
void BVHRT::BVH2TraverseF32WithRayInfo(const float3 ray_pos, const float3 ray_dir, float tNear,
                                uint32_t instId, uint32_t geomId, bool stopOnFirstHit,
                                CRT_Hit* pHit)
{
  const uint32_t bvhOffset = m_geomData[geomId].bvhOffset;
  ...
  while (top >= 0 && !hitFound)
  {
    while (top >= 0 && ((leftNodeOffset & LEAF_BIT) == 0))
    {
      ...
      if (hitChild0 && !hitChild1)
      {
        RI.bboxes_info.push_back({fatNode.left.boxMin, fatNode.left.boxMax, node0_leftOffset});
      }
      else if (!hitChild0 && hitChild1)
      {
        RI.bboxes_info.push_back({fatNode.right.boxMin, fatNode.right.boxMax, node1_leftOffset});
      }
      else if (hitChild0 && hitChild1)
      {
        if (tm0.x < tm1.x)
