...
void MultiRenderer_slang_comp_rq::UpdatePlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  ...
  m_uboData.m_pAccelStruct_remap_size     = uint32_t( m_pAccelStruct_remap.size() );     assert( m_pAccelStruct_remap.size() < maxAllowedSize );
  m_uboData.m_pAccelStruct_remap_capacity = uint32_t( m_pAccelStruct_remap.capacity() ); assert( m_pAccelStruct_remap.capacity() < maxAllowedSize );
...
void MultiRenderer_slang_comp_rq::ReadPlainMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  ...
  m_pAccelStruct_remap.resize(m_uboData.m_pAccelStruct_remap_size);
...
void MultiRenderer_slang_comp_rq::UpdateVectorMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  if(m_pAccelStruct_remap.size() > 0)
    a_pCopyEngine->UpdateBuffer(m_vdata.m_pAccelStruct_remapBuffer, 0, m_pAccelStruct_remap.data(), m_pAccelStruct_remap.size()*sizeof(uint2) );
...
void MultiRenderer_slang_comp_rq::Update_m_lights(size_t a_first, size_t a_size)
{
...
void MultiRenderer_slang_comp_rq::UpdateTextureMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  ...
  if (vkBeginCommandBuffer(cmdBuff, &beginInfo) != VK_SUCCESS)
    throw std::runtime_error("MultiRenderer_slang_comp_rq::UpdateTextureMembers: failed to begin command buffer!");
...
void MultiRenderer_slang_comp_rq::ResolveDebugCmd(uint32_t count, uint32_t sample_id, uint32_t render_mode, LiteMath::float4* out_color)
{
...
void MultiRenderer_slang_comp_rq::FillGbufferCmd(uint32_t count, uint32_t sample_id)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::FillGbufferWithTransparencyCmd(uint32_t count, uint32_t sample_id)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::ResolveOutlineCmd(uint32_t count, uint32_t sample_id, uint32_t render_mode, float4* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::AverageColorCmd(uint32_t count, uint32_t sample_count, float4* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::TonemapCmd(uint32_t count, uint32_t sample_count, uint32_t* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::PackXYCmd(uint32_t w, uint32_t h)
{
...
void MultiRenderer_slang_comp_rq::ResolveCommonCmd(uint32_t count, uint32_t sample_id, uint32_t render_mode, LiteMath::float4* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::ResolveCommonWithTransparencyCmd(uint32_t count, uint32_t sample_id, uint32_t render_mode, LiteMath::float4* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::ResolveCTFCmd(uint32_t count, uint32_t sample_id, uint32_t render_mode, LiteMath::float4* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void MultiRenderer_slang_comp_rq::copyKernelFloatCmd(uint32_t length)
{
...
void MultiRenderer_slang_comp_rq::matMulTransposeCmd(uint32_t A_offset, uint32_t B_offset, uint32_t C_offset, uint32_t A_col_len, uint32_t B_col_len, uint32_t A_row_len)
{
...
VkBufferMemoryBarrier MultiRenderer_slang_comp_rq::BarrierForClearFlags(VkBuffer a_buffer)
{
...
VkBufferMemoryBarrier MultiRenderer_slang_comp_rq::BarrierForSingleBuffer(VkBuffer a_buffer)
{
  VkBufferMemoryBarrier bar = {};
  bar.sType               = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
  bar.pNext               = NULL;
...
void MultiRenderer_slang_comp_rq::BarriersForSeveralBuffers(VkBuffer* a_inBuffers, VkBufferMemoryBarrier* a_outBarriers, uint32_t a_buffersNum)
{
...
void MultiRenderer_slang_comp_rq::RenderFloatCmd(VkCommandBuffer a_commandBuffer, float4* a_outColor, uint32_t a_width, uint32_t a_height, int a_passNum)
{
...
void MultiRenderer_slang_comp_rq::RenderCmd(VkCommandBuffer a_commandBuffer, uint32_t* a_outColor, uint32_t a_width, uint32_t a_height, int a_passNum)
{
  VkPipelineStageFlagBits prevStageBits = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT };
...
void MultiRenderer_slang_comp_rq::ClearCmd(VkCommandBuffer a_commandBuffer, uint32_t a_width, uint32_t a_height)
{
  VkPipelineStageFlagBits prevStageBits = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT;
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT };
...
void MultiRenderer_slang_comp_rq::RenderFloat(float4* a_outColor, uint32_t a_width, uint32_t a_height, int a_passNum)
{
...
void MultiRenderer_slang_comp_rq::Render(uint32_t* a_outColor, uint32_t a_width, uint32_t a_height, int a_passNum)
{
  // (1) get global Vulkan context objects
  //
  VkInstance       instance       = m_ctx.instance;
...
void MultiRenderer_slang_comp_rq::Clear(uint32_t a_width, uint32_t a_height)
{
  // (1) get global Vulkan context objects
  //
  VkInstance       instance       = m_ctx.instance;
...
void MultiRenderer_slang_comp_rq::GetExecutionTime(const char* a_funcName, float a_out[4])
{
