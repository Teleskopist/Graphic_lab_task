...
void check_model(const std::string &path)
{
	...
	std::string model_name = split(split(path, '/').back(), '.')[0];
	...
	for (int depth = 3; depth < max_depth; depth++)
	{
		...
		co_settings.brick_size = settings.brick_size;
		co_settings.brick_pad = settings.brick_pad;
		...
		auto pRender = CreateMultiRenderer(DEVICE_GPU);
		preset.normal_mode = settings.brick_pad == 1 ? NORMAL_MODE_SDF_SMOOTHED : NORMAL_MODE_VERTEX;
		pRender->SetPreset(preset);
...
namespace model_validator
{
	...
	*/
	size_t check_model(const cmesh4::SimpleMesh &mesh, const std::string &image_dir)
	{
		...
		{
			float r = 3;
			float theta = 2 * LiteMath::M_PI * i / cameras.size();
