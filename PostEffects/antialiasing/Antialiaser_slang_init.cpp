...
std::shared_ptr<Antialiaser> CreateAntialiaser_slang(vk_utils::VulkanContext a_ctx, size_t a_maxThreadsGenerated)
{
  auto pObj = std::make_shared<Antialiaser_slang>();
  pObj->SetVulkanContext(a_ctx);
  pObj->InitVulkanObjects(a_ctx.device, a_ctx.physicalDevice, a_maxThreadsGenerated);
  return pObj;
}

vk_utils::VulkanDeviceFeatures Antialiaser_slang_ListRequiredDeviceFeatures()
{
  vk_utils::VulkanDeviceFeatures res;
  res.features2 = Antialiaser_slang::ListRequiredDeviceFeatures(res.extensionNames);
  res.apiVersion = VK_API_VERSION_1_1;
  return res;
}

void Antialiaser_slang::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
...
VkBufferUsageFlags Antialiaser_slang::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t Antialiaser_slang::GetDefaultMaxTextures() const { return 256; }

void Antialiaser_slang::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void Antialiaser_slang::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
void Antialiaser_slang::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
  vkDestroyBuffer(m_device, m_classDataBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.m_AABufferBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.m_AABuffer1Buffer, nullptr);
...
Antialiaser_slang::~Antialiaser_slang()
{
  ...
  vkDestroyDescriptorSetLayout(m_device, blendDSLayout, nullptr);
  blendDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorPool(m_device, m_dsPool, NULL); m_dsPool = VK_NULL_HANDLE;
  DeleteDeviceData();
}

void Antialiaser_slang::InitHelpers()
{
  vkGetPhysicalDeviceProperties(m_physicalDevice, &m_devProps);
}


void Antialiaser_slang::InitKernel_blend(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_slang/kernel2D_blend.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  blendDSLayout = CreateblendDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, blendDSLayout, &blendLayout, &blendPipeline);
  }
  else
  {
    blendLayout   = nullptr;
    blendPipeline = nullptr;
  }
}


void Antialiaser_slang::InitKernels(const char* a_filePath)
{
  InitKernel_blend(a_filePath);
}

void Antialiaser_slang::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
...
void Antialiaser_slang::ReserveEmptyVectors()
{
  if(m_AABuffer.capacity() == 0)
    m_AABuffer.reserve(4);
  if(m_AABuffer1.capacity() == 0)
    m_AABuffer1.reserve(4);
}

void Antialiaser_slang::InitDeviceData()
{
  ...
  m_vdata.m_AABufferBuffer = vk_utils::createBuffer(m_device, m_AABuffer.capacity()*sizeof(float4), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.m_AABufferBuffer);
  m_vdata.m_AABuffer1Buffer = vk_utils::createBuffer(m_device, m_AABuffer1.capacity()*sizeof(float4), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  memberVectors.push_back(m_vdata.m_AABuffer1Buffer);
...
void Antialiaser_slang::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[Antialiaser_slang::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
Antialiaser_slang::MemLoc Antialiaser_slang::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
Antialiaser_slang::MemLoc Antialiaser_slang::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "Antialiaser_slang::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void Antialiaser_slang::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void Antialiaser_slang::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 Antialiaser_slang::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
Antialiaser_slang::MegaKernelIsEnabled Antialiaser_slang::m_megaKernelFlags;
