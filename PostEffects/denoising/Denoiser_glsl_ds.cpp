...
void Denoiser_glsl::AllocateAllDescriptorSets()
{
  // allocate pool
  //
  ...
  buffersSize.descriptorCount = 4 + 64 ; //
  ...
  layouts[0] = filterFloatDSLayout;
  layouts[1] = filterDSLayout;
...
VkDescriptorSetLayout Denoiser_glsl::CreatefilterDSLayout()
{
...
VkDescriptorSetLayout Denoiser_glsl::CreatefilterFloatDSLayout()
{
  std::array<VkDescriptorSetLayoutBinding, 2+1> dsBindings;
...
VkDescriptorSetLayout Denoiser_glsl::CreatecopyKernelFloatDSLayout()
{
...
VkDescriptorSetLayout Denoiser_glsl::CreatematMulTransposeDSLayout()
{
...
void Denoiser_glsl::UpdateAllGeneratedDescriptorSets_DenoiseFloat()
{
  // now create actual bindings
  //
  // descriptor set #0: filterFloatCmd (["in_data","out_data"])
  {
    ...
    descriptorBufferInfo[0].buffer = DenoiseFloat_local.in_dataBuffer;
    descriptorBufferInfo[0].offset = DenoiseFloat_local.in_dataOffset;
    ...
    descriptorBufferInfo[1].buffer = DenoiseFloat_local.out_dataBuffer;
    descriptorBufferInfo[1].offset = DenoiseFloat_local.out_dataOffset;
...
void Denoiser_glsl::UpdateAllGeneratedDescriptorSets_Denoise()
{
  // now create actual bindings
  //
  // descriptor set #1: filterCmd (["in_data","out_data"])
  {
    constexpr uint additionalSize = 1;
    ...
    descriptorBufferInfo[0]        = VkDescriptorBufferInfo{};
    descriptorBufferInfo[0].buffer = Denoise_local.in_dataBuffer;
    descriptorBufferInfo[0].offset = Denoise_local.in_dataOffset;
    descriptorBufferInfo[0].range  = VK_WHOLE_SIZE;
    writeDescriptorSet[0]                  = VkWriteDescriptorSet{};
    writeDescriptorSet[0].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    ...
    descriptorBufferInfo[1]        = VkDescriptorBufferInfo{};
    descriptorBufferInfo[1].buffer = Denoise_local.out_dataBuffer;
    descriptorBufferInfo[1].offset = Denoise_local.out_dataOffset;
    descriptorBufferInfo[1].range  = VK_WHOLE_SIZE;
    writeDescriptorSet[1]                  = VkWriteDescriptorSet{};
    writeDescriptorSet[1].sType            = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
