...
std::shared_ptr<Denoiser> CreateDenoiser_glsl(vk_utils::VulkanContext a_ctx, size_t a_maxThreadsGenerated)
{
  auto pObj = std::make_shared<Denoiser_glsl>();
  pObj->SetVulkanContext(a_ctx);
  pObj->InitVulkanObjects(a_ctx.device, a_ctx.physicalDevice, a_maxThreadsGenerated);
  return pObj;
}

vk_utils::VulkanDeviceFeatures Denoiser_glsl_ListRequiredDeviceFeatures()
{
  vk_utils::VulkanDeviceFeatures res;
  res.features2 = Denoiser_glsl::ListRequiredDeviceFeatures(res.extensionNames);
  res.apiVersion = VK_API_VERSION_1_1;
  return res;
}

void Denoiser_glsl::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
...
VkBufferUsageFlags Denoiser_glsl::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t Denoiser_glsl::GetDefaultMaxTextures() const { return 256; }

void Denoiser_glsl::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void Denoiser_glsl::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
void Denoiser_glsl::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
...
Denoiser_glsl::~Denoiser_glsl()
{
  ...
  vkDestroyDescriptorSetLayout(m_device, filterDSLayout, nullptr);
  filterDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorSetLayout(m_device, filterFloatDSLayout, nullptr);
  filterFloatDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorPool(m_device, m_dsPool, NULL); m_dsPool = VK_NULL_HANDLE;
  DeleteDeviceData();
}

void Denoiser_glsl::InitHelpers()
{
  vkGetPhysicalDeviceProperties(m_physicalDevice, &m_devProps);
}


void Denoiser_glsl::InitKernel_filter(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_glsl/kernel2D_filter.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  filterDSLayout = CreatefilterDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, filterDSLayout, &filterLayout, &filterPipeline);
  }
  else
  {
...
void Denoiser_glsl::InitKernel_filterFloat(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_glsl/kernel2D_filterFloat.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  filterFloatDSLayout = CreatefilterFloatDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, filterFloatDSLayout, &filterFloatLayout, &filterFloatPipeline);
  }
  else
  {
    filterFloatLayout   = nullptr;
    filterFloatPipeline = nullptr;
  }
}


void Denoiser_glsl::InitKernels(const char* a_filePath)
{
  InitKernel_filter(a_filePath);
  InitKernel_filterFloat(a_filePath);
}

void Denoiser_glsl::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
...
void Denoiser_glsl::ReserveEmptyVectors()
{
}

void Denoiser_glsl::InitDeviceData()
{
...
void Denoiser_glsl::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[Denoiser_glsl::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
Denoiser_glsl::MemLoc Denoiser_glsl::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
Denoiser_glsl::MemLoc Denoiser_glsl::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "Denoiser_glsl::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void Denoiser_glsl::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void Denoiser_glsl::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 Denoiser_glsl::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
Denoiser_glsl::MegaKernelIsEnabled Denoiser_glsl::m_megaKernelFlags;
