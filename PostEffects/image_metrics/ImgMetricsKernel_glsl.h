...
struct ImgMetricsKernel_glsl_UBO_Data
{
  float m_executionTime;
  float m_summ;
  uint tmpred04_capacity;
  uint tmpred04_size;
  uint dummy_last;
};
/////////////////////////////////////////////////////////////////////////////////////////// UBO
class ImgMetricsKernel_glsl : public ImgMetricsKernel
{
public:

  ImgMetricsKernel_glsl()
  {
  }
  ...
  virtual void SetVulkanInOutFor_CalcArraySumm(
    VkBuffer rt_dataBuffer,
    size_t   rt_dataOffset,
    VkBuffer ref_dataBuffer,
    size_t   ref_dataOffset,
    uint32_t dummyArgument = 0)
  {
    CalcArraySumm_local.rt_dataBuffer = rt_dataBuffer;
    CalcArraySumm_local.rt_dataOffset = rt_dataOffset;
    CalcArraySumm_local.ref_dataBuffer = ref_dataBuffer;
    CalcArraySumm_local.ref_dataOffset = ref_dataOffset;
    UpdateAllGeneratedDescriptorSets_CalcArraySumm();
  }

  virtual ~ImgMetricsKernel_glsl();
...
virtual void UpdateDeviceData(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)   {
  ...
  virtual void CalcArraySummCmd(VkCommandBuffer a_commandBuffer, const uint32_t* rt_data, const uint32_t* ref_data, size_t dataSize);

  void CalcArraySumm(const uint32_t* rt_data, const uint32_t* ref_data, size_t dataSize) override;

  inline vk_utils::ExecTime GetCalcArraySummExecutionTime() const { return m_exTimeCalcArraySumm; }

  vk_utils::ExecTime m_exTimeCalcArraySumm;
  ...
  virtual void ArraySummCmd(const uint32_t* rt_data, const uint32_t* ref_data, size_t dataSize);
...
protected:
  ...
  virtual void UpdateAllGeneratedDescriptorSets_CalcArraySumm();
  ...
  virtual std::string AlterShaderPath(const char* in_shaderPath) { return std::string("PostEffects/image_metrics/") + std::string(in_shaderPath); }
  
  

  struct CalcArraySumm_Data
  {
    VkBuffer rt_dataBuffer = VK_NULL_HANDLE;
    size_t   rt_dataOffset = 0;
    VkBuffer ref_dataBuffer = VK_NULL_HANDLE;
    size_t   ref_dataOffset = 0;
  ...
  {
    VkBuffer tmpred04Buffer = VK_NULL_HANDLE;
    size_t   tmpred04Offset = 0;
  } m_vdata;
  ...
  VkPipelineLayout      ArraySummLayout   = VK_NULL_HANDLE;
  VkPipeline            ArraySummPipeline = VK_NULL_HANDLE;
  VkDescriptorSetLayout ArraySummDSLayout = VK_NULL_HANDLE;
  VkPipeline            ArraySummInitPipeline = VK_NULL_HANDLE;
  VkPipeline            ArraySummReductionPipeline = VK_NULL_HANDLE;
  VkDescriptorSetLayout CreateArraySummDSLayout();
  virtual void InitKernel_ArraySumm(const char* a_filePath);
  ...
  ImgMetricsKernel_glsl_UBO_Data m_uboData;
