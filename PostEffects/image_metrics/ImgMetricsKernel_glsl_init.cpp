...
std::shared_ptr<ImgMetricsKernel> CreateImgMetricsKernel_glsl(vk_utils::VulkanContext a_ctx, size_t a_maxThreadsGenerated)
{
  auto pObj = std::make_shared<ImgMetricsKernel_glsl>();
  pObj->SetVulkanContext(a_ctx);
  pObj->InitVulkanObjects(a_ctx.device, a_ctx.physicalDevice, a_maxThreadsGenerated);
  return pObj;
}

vk_utils::VulkanDeviceFeatures ImgMetricsKernel_glsl_ListRequiredDeviceFeatures()
{
  vk_utils::VulkanDeviceFeatures res;
  res.features2 = ImgMetricsKernel_glsl::ListRequiredDeviceFeatures(res.extensionNames);
  res.apiVersion = VK_API_VERSION_1_1;
  return res;
}

void ImgMetricsKernel_glsl::InitVulkanObjects(VkDevice a_device, VkPhysicalDevice a_physicalDevice, size_t a_maxThreadsCount)
{
...
VkBufferUsageFlags ImgMetricsKernel_glsl::GetAdditionalFlagsForUBO() const
{
  return VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
}

uint32_t ImgMetricsKernel_glsl::GetDefaultMaxTextures() const { return 256; }

void ImgMetricsKernel_glsl::MakeComputePipelineAndLayout(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout* pPipelineLayout, VkPipeline* pPipeline)
{
...
void ImgMetricsKernel_glsl::MakeComputePipelineOnly(const char* a_shaderPath, const char* a_mainName, const VkSpecializationInfo *a_specInfo, const VkDescriptorSetLayout a_dsLayout, VkPipelineLayout pipelineLayout, VkPipeline* pPipeline)
{
  VkPipelineShaderStageCreateInfo shaderStageInfo = {};
  shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
...
void ImgMetricsKernel_glsl::DeleteDeviceData()
{
  if(m_commitCount == 0)
    return;
  vkDestroyBuffer(m_device, m_classDataBuffer, nullptr);
  vkDestroyBuffer(m_device, m_vdata.tmpred04Buffer, nullptr);
...
ImgMetricsKernel_glsl::~ImgMetricsKernel_glsl()
{
  ...
  vkDestroyDescriptorSetLayout(m_device, ArraySummDSLayout, nullptr);
  ArraySummDSLayout = VK_NULL_HANDLE;
  vkDestroyDescriptorPool(m_device, m_dsPool, NULL); m_dsPool = VK_NULL_HANDLE;
  DeleteDeviceData();
}

void ImgMetricsKernel_glsl::InitHelpers()
{
  vkGetPhysicalDeviceProperties(m_physicalDevice, &m_devProps);
}


void ImgMetricsKernel_glsl::InitKernel_ArraySumm(const char* a_filePath)
{
  std::string shaderPath = AlterShaderPath("shaders_glsl/kernel1D_ArraySumm.comp.spv");
  const VkSpecializationInfo* kspec = nullptr;
  ArraySummDSLayout = CreateArraySummDSLayout();
  if(true)
  {
    MakeComputePipelineAndLayout(shaderPath.c_str(), "main", kspec, ArraySummDSLayout, &ArraySummLayout, &ArraySummPipeline);
  }
  else
  {
    ArraySummLayout   = nullptr;
    ArraySummPipeline = nullptr;
  }
  shaderPath = AlterShaderPath("shaders_glsl/kernel1D_ArraySumm_Reduction.comp.spv");
  MakeComputePipelineOnly(shaderPath.c_str(), "main", kspec, ArraySummDSLayout, ArraySummLayout, &ArraySummReductionPipeline);
  shaderPath = AlterShaderPath("shaders_glsl/kernel1D_ArraySumm_Init.comp.spv");
  MakeComputePipelineOnly(shaderPath.c_str(), "main", kspec, ArraySummDSLayout, ArraySummLayout, &ArraySummInitPipeline);
}


void ImgMetricsKernel_glsl::InitKernels(const char* a_filePath)
{
  InitKernel_ArraySumm(a_filePath);
}

void ImgMetricsKernel_glsl::InitBuffers(size_t a_maxThreadsCount, bool a_tempBuffersOverlay)
{
  ReserveEmptyVectors();
  ...
  {
    const size_t sizeOfBuffer = ComputeReductionAuxBufferElements(a_maxThreadsCount, REDUCTION_BLOCK_SIZE)*sizeof(float);
    m_vdata.tmpred04Buffer = vk_utils::createBuffer(m_device, sizeOfBuffer, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT);
    allBuffersRef.push_back(m_vdata.tmpred04Buffer);
  }
...
void ImgMetricsKernel_glsl::ReserveEmptyVectors()
{
}

void ImgMetricsKernel_glsl::InitDeviceData()
{
...
void ImgMetricsKernel_glsl::AssignBuffersToMemory(const std::vector<VkBuffer>& a_buffers, VkDeviceMemory a_mem)
{
  ...
  for(size_t i=1;i<memInfos.size();i++)
  {
    ...
    {
      std::cout << "[ImgMetricsKernel_glsl::AssignBuffersToMemory]: error, input buffers has different 'memReq.memoryTypeBits'" << std::endl;
      return;
    }
  }
...
ImgMetricsKernel_glsl::MemLoc ImgMetricsKernel_glsl::AllocAndBind(const std::vector<VkBuffer>& a_buffers, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_buffers.size() > 0)
  {
...
ImgMetricsKernel_glsl::MemLoc ImgMetricsKernel_glsl::AllocAndBind(const std::vector<VkImage>& a_images, VkMemoryAllocateFlags a_flags)
{
  MemLoc currLoc;
  if(a_images.size() > 0)
  {
    ...
    for(size_t i=0; i<reqs.size(); i++)
    {
      ...
      {
        std::cout << "ImgMetricsKernel_glsl::AllocAndBind(textures): memoryTypeBits warning, need to split mem allocation (override me)" << std::endl;
        break;
      }
    }
...
void ImgMetricsKernel_glsl::FreeAllAllocations(std::vector<MemLoc>& a_memLoc)
{
...
void ImgMetricsKernel_glsl::AllocMemoryForMemberBuffersAndImages(const std::vector<VkBuffer>& a_buffers, const std::vector<VkImage>& a_images)
{
...
VkPhysicalDeviceFeatures2 ImgMetricsKernel_glsl::ListRequiredDeviceFeatures(std::vector<const char*>& deviceExtensions)
{
...
ImgMetricsKernel_glsl::MegaKernelIsEnabled ImgMetricsKernel_glsl::m_megaKernelFlags;
