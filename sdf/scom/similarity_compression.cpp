...
#pragma omp parallel for schedule(static)
      for (int r_id = 0; r_id < rot_count; r_id++)
      {
        ...
        dataset.data_points[cur_point_group * rot_count + r_id].transform_id = r_id + rot_start;
  ...
  void create_BallTreeAS(const Dataset &dataset, std::unique_ptr<nn_search::INNSearchAS> &NN_search_AS)
  {
    NN_search_AS.reset(new nn_search::BallTreeL2());
  ...
  void similarity_compression_advanced(const sdf_converter::GlobalOctree &g_octree, const Settings &settings, CompressionOutput &output)
  {
    ...
    std::unique_ptr<nn_search::INNSearchAS> NN_search_AS;

    if (settings.search_algorithm == SearchAlgorithm::LINEAR_SEARCH)
    {
      NN_search_AS.reset(new nn_search::LinearSearchL2AS());
