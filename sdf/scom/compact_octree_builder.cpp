...
static std::atomic<unsigned> stat_leaf_count(0);
static std::atomic<unsigned> stat_leaf_bits(0);
static std::atomic<unsigned> stat_leaf_bits_distances(0);
static std::atomic<unsigned> stat_leaf_bits_other(0);

static std::atomic<unsigned> stat_nonleaf_count(0);
static std::atomic<unsigned> stat_nonleaf_bits(0);
static std::atomic<unsigned> stat_nonleaf_bits_transforms(0);
static std::atomic<unsigned> stat_nonleaf_bits_other(0);
...
#define P_OFF(off, i, j, k) off + (i) * p_size *p_size + (j) * p_size + (k)
          ...
          if (min_val <= 0 && max_val >= 0)
          {
                ...
                k >= 0 && k < header.brick_size)
            {
              ...
              for (int e = 0; e < 4; e++)
              {
                ...
                // y axis
                if (ctx.values_tmp[V_OFF(off, 0, e / 2, e % 2)] * ctx.values_tmp[V_OFF(off, 1, e / 2, e % 2)] < 0)
                {
                  ctx.requirement_flags[P_OFF(p_off, 0, e / 2 - 1, e % 2 - 1)] = true;
                  ctx.requirement_flags[P_OFF(p_off, 0, e / 2 - 1, e % 2)] = true;
                  ctx.requirement_flags[P_OFF(p_off, 0, e / 2, e % 2 - 1)] = true;
                  ctx.requirement_flags[P_OFF(p_off, 0, e / 2, e % 2 - 1)] = true;
                }

                // y axis
                if (ctx.values_tmp[V_OFF(off, e / 2, 0, e % 2)] * ctx.values_tmp[V_OFF(off, e >> 1, 1, e & 1)] < 0)
                {
                  ctx.requirement_flags[P_OFF(p_off, e / 2 - 1, 0, e % 2 - 1)] = true;
                  ctx.requirement_flags[P_OFF(p_off, e / 2 - 1, 0, e % 2)] = true;
                  ctx.requirement_flags[P_OFF(p_off, e / 2, 0, e % 2 - 1)] = true;
                  ctx.requirement_flags[P_OFF(p_off, e / 2, 0, e % 2)] = true;
                }

                // z axis
                if (ctx.values_tmp[V_OFF(off, e / 2, e % 2, 0)] * ctx.values_tmp[V_OFF(off, e / 2, e % 2, 1)] < 0)
                {
                  ctx.requirement_flags[P_OFF(p_off, e / 2 - 1, e % 2, 0)] = true;
                  ctx.requirement_flags[P_OFF(p_off, e / 2 - 1, e % 2 - 1, 0)] = true;
                  ctx.requirement_flags[P_OFF(p_off, e / 2, e % 2, 0)] = true;
                  ctx.requirement_flags[P_OFF(p_off, e / 2, e % 2 - 1, 0)] = true;
                }
              }
            }
          }
    ...
    if (header.uv_size > 0)
    {
      ...
      for (int i = 0; i < 8; i++)
      {
        if (octree.header.tc_channel_id >= 0)
        {
          ...
          tc_x = tc_data.data_f[idx * tc_data.num_components * 8 + i * 2 + 0];
          tc_y = tc_data.data_f[idx * tc_data.num_components * 8 + i * 2 + 1];
        }
        float u_comp = 0xFFFF * LiteMath::clamp(tc_x/*octree.nodes[idx].tex_coords[i].x*/, 0.0f, 1.0f);
        float v_comp = 0xFFFF * LiteMath::clamp(tc_y/*octree.nodes[idx].tex_coords[i].y*/, 0.0f, 1.0f);
  ...
  uint2 brick_values_compress(unsigned leaf_type,
                              ...
                              float add_min, float add_max)
  {
    ...
    return uint2(total_size_uint, distances_size_uint);
  }
  ...
  void check_compact_octree_v3_node_packing(const GlobalOctree &octree, COctreeV3Header header,
                                            ...
                                            unsigned ofs, unsigned ch_idx)
  {
    unsigned compact_leaf_idx = global_ctx.node_id_cleaf_id_remap[ofs + ch_idx];
    ...
    uint3 ch_p = 2 * task.p + uint3((ch_idx & 4) >> 2, (ch_idx & 2) >> 1, ch_idx & 1);

    unsigned leaf_size = brick_values_compress_no_packing(header, thread_ctx, ofs + ch_idx, 2 * task.lod_size, ch_p);
    unsigned a_leaf_size = brick_values_compress(0, octree, header, thread_ctx, ofs + ch_idx, 2 * task.lod_size, ch_p, add_min, add_max).x;
                                     ...
                                     unsigned thread_id, const COctreeV3BuildTask &task)
  {
    ...
    if (global_ctx.compact_leaf_offsets[compact_leaf_idx] == 0u) // this is a new node, we should compress it and add to compact octree
    {
      ...
      uint2 ls_ds = brick_values_compress(header.default_leaf_type, octree, header, thread_ctx, task.nodeId, task.lod_size, task.p, add_min, add_max);
      leaf_size = ls_ds.x;
      ...
      if (leaf_size >= uncompressed_leaf_size && header.fallback_leaf_type != header.default_leaf_type)
      {
        ...
        ls_ds = brick_values_compress(header.fallback_leaf_type, octree, header, thread_ctx, task.nodeId, task.lod_size, task.p, add_min, add_max);
        leaf_size = ls_ds.x;
      ...
      stat_leaf_bits += 32 * leaf_size;
      stat_leaf_bits_distances += 32 * ls_ds.y;
      stat_leaf_bits_other += 32 * (leaf_size - ls_ds.y);
      stat_leaf_count++;
    }
                                     ...
                                     unsigned thread_id, const COctreeV3BuildTask &task)
  {
    ...
    int bits_per_transform = header.node_pack_mode == COCTREE_NODE_PACK_MODE_SIM_COMP_FULL ? 32 : 14;
    stat_nonleaf_bits += 32 * total_uints;
    stat_nonleaf_bits_transforms += bits_per_transform*ch_count;
    stat_nonleaf_bits_other += 32*total_uints - bits_per_transform*ch_count;
    stat_nonleaf_count++;
  ...
  void global_octree_calculate_node_levels_count_rec(const GlobalOctree &octree, std::vector<unsigned> &node_levels, unsigned idx, unsigned level)
  {
    if (octree.nodes[idx].type == GlobalOctreeNodeType::NODE || octree.nodes[idx].type == GlobalOctreeNodeType::EMPTY_NODE)
    {
      node_levels[level]++;
      for (int i = 0; i < 8; i++)
        global_octree_calculate_node_levels_count_rec(octree, node_levels, octree.nodes[idx].offset + i, level + 1);
    }
  }
                                  ...
                                  COctreeV3Settings co_settings, scom::Settings settings)
  {
    ...
    stat_leaf_bits.store(0);
    stat_leaf_bits_distances.store(0);
    stat_leaf_bits_other.store(0);
    stat_leaf_count.store(0);

    stat_nonleaf_bits.store(0);
    stat_nonleaf_bits_transforms.store(0);
    stat_nonleaf_bits_other.store(0);
    stat_nonleaf_count.store(0);
    ...
    if (co_settings.print_build_stats)
    {
      printf("similarity compression total:%8.1f ms\n", 1e-6f*std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count());
      printf("prepare compression tasks:   %8.1f ms\n", 1e-6f*std::chrono::duration_cast<std::chrono::nanoseconds>(t3 - t2).count());
      printf("compress leaves:             %8.1f ms\n", 1e-6f*std::chrono::duration_cast<std::chrono::nanoseconds>(t4 - t3).count());
      printf("compress nodes:              %8.1f ms\n", 1e-6f*std::chrono::duration_cast<std::chrono::nanoseconds>(t5 - t4).count());

      printf("compact octree %.1f Kb leaf, %.1f Kb non-leaf\n", stat_leaf_bits.load() / 8192.0f, stat_nonleaf_bits.load() / 8192.0f);
    
      std::vector<unsigned> nl_counts(16, 0);
      global_octree_calculate_node_levels_count_rec(octree, nl_counts, 0, 0);

      printf("leaf types  [%d %d %d %d]\n", type_stat[0].load(), type_stat[1].load(), type_stat[2].load(), type_stat[3].load());
      printf("node levels [");
      for (int i = 0; i < nl_counts.size(); i++)
        printf("%u ", nl_counts[i]);
      printf("]\n");

      float sz = 32*compact_octree.data.size();
      printf("size               %.1f Kb\n", sz / 8192.0f);
      printf("- nodes            %.1f Kb (%.1f%%)\n", stat_nonleaf_bits.load() / 8192.0f, 100.0f * stat_nonleaf_bits.load() / sz);
      printf("-- transform codes %.1f Kb (%.1f%%)\n", stat_nonleaf_bits_transforms.load() / 8192.0f, 100.0f * stat_nonleaf_bits_transforms.load() / sz);
      printf("-- other           %.1f Kb (%.1f%%)\n", stat_nonleaf_bits_other.load() / 8192.0f, 100.0f * stat_nonleaf_bits_other.load() / sz);
      printf("- leaves           %.1f Kb (%.1f%%)\n", stat_leaf_bits.load() / 8192.0f, 100.0f * stat_leaf_bits.load() / sz);
      printf("-- distances       %.1f Kb (%.1f%%)\n", stat_leaf_bits_distances.load() / 8192.0f, 100.0f * stat_leaf_bits_distances.load() / sz);
      printf("-- other           %.1f Kb (%.1f%%)\n", stat_leaf_bits_other.load() / 8192.0f, 100.0f * stat_leaf_bits_other.load() / sz);
    }
