...
struct Camera
{
  Camera() : pos(0.0f, 0.0f, +5.0f), lookAt(0, 0, 0), up(0, 1, 0), fovy(45.0f), 
             tdist(100.0f), z_near(0.001f), z_far(100.0f) {}
  ...
  void offsetOrientation(float a_upAngle, float rightAngle)
  {
    ...
    if (rightAngle != 0.0f)  // rotate horizontal
    {
      ...
      float3 direction2 = LiteMath::normalize(to_float3(mul(rot, to_float4(forward(),1.0f))));
      up     = normalize(                     to_float3(mul(rot, to_float4(up,1.0f))));
