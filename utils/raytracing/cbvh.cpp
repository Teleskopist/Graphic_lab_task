...
static constexpr uint32_t START_MASK = 0x0FFFFFFF;
static constexpr uint32_t END_MASK   = 0xF0000000;
static constexpr uint32_t SIZE_MASK  = 0x70000000;
...
{
  return LEAF_BIT | ((size << 28) & SIZE_MASK) | (start & START_MASK);
}
...
struct Converter2to4
{
  ...
  static inline uint32_t EXTRACT_COUNT(uint32_t a_leftOffset) { return (a_leftOffset & SIZE_MASK) >> 28; }
                       ...
                       BuilderPresets a_presets)
{
  ...
  //else 
  //  bvhData = BuildBVHSimple(inVertices, a_vertNum, a_indices, a_indexNum, presets1);
  auto t2 = std::chrono::high_resolution_clock::now();

  float build_time = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
  //printf("[BuildBVH] Primitives = %lu. Build time = %.1f ms\n", a_indexNum/3, build_time);
