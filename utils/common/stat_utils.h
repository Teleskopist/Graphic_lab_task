...
namespace stat_utils
{
  ...
  template <typename T>
  double length(const std::vector<T> &a)
  {
    long double sum = 0;
    for (int i = 0; i < a.size(); i++)
    {
  ...
  template <typename T>
  double min(const std::vector<T> &a)
  {
    assert(a.size() > 0);
  ...
  template <typename T>
  double max(const std::vector<T> &a)
  {
    assert(a.size() > 0);
  ...
  template <typename T>
  double variance(const std::vector<T> &a)
  {
    assert(a.size() > 1);
  ...
  template <typename T>
  double quantile(const std::vector<T> &a, double quantile)
  {
    assert(a.size() > 0);
    assert(quantile > 0 && quantile < 1);
    std::vector<T> a_copy = a;
    std::sort(a_copy.begin(), a_copy.end());
    return a_copy[(int)(a.size() * quantile)];
  }

  template <typename T>
  double median(const std::vector<T> &a)
  {
    return quantile(a, 0.5f);
  }

  static double z_score(double confidence_level)
  {
    ...
    {
      z += (1.0 / sqrt(2.0 * 3.14159)) * exp(-(x * x) / 2.0) * step;
      x += step;
    }
  ...
  template <typename T>
  double confidence(const std::vector<T> &a, double confidence_level = 0.95f)
  {
    assert(a.size() > 1);
