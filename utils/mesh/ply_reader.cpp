...
namespace cmesh4
{
  ...
  public:
    ...
    PlyToken NextToken();
    std::vector<float> ReadValues(PlyToken::Type type, int count);
    std::vector<int> ReadIndices(PlyToken::Type type, int count);
  ...
  PlyToken PlyTokenizer::NextToken()
  {
    if (eof)
    {
  ...
  std::vector<float> PlyTokenizer::ReadValues(PlyToken::Type type, int count)
  {
    std::vector<float> res(count, 0.0f);

    if (read_binary)
    {
      if (type == PlyToken::DOUBLE || type == PlyToken::FLOAT64)
      {
      ...
      else if (type == PlyToken::FLOAT || type == PlyToken::FLOAT32)
      {
    ...
    {
      if (type == PlyToken::DOUBLE || type == PlyToken::FLOAT64 || type == PlyToken::FLOAT || type == PlyToken::FLOAT32)
      {
        for (int i = 0; i < count; ++i)
        {
          PlyToken token = NextToken();
          if (token.type == PlyToken::VALUE)
          {
            res[i] = token.value;
          }
          else
          {
            printf("[ply_reader::ERROR] Got token (%d) instead of PlyToken::VALUE for ReadValues<float>\n", token.type);
            break;
          }
        }
      }
  ...
  std::vector<int> PlyTokenizer::ReadIndices(PlyToken::Type type, int count)
  {
    std::vector<int> res(count, -1);

    if (read_binary)
    {
      if (type == PlyToken::CHAR || type == PlyToken::INT8)
      {
      ...
      else if (type == PlyToken::UCHAR || type == PlyToken::UINT8)
      {
      ...
      else if (type == PlyToken::SHORT || type == PlyToken::INT16)
      {
      ...
      else if (type == PlyToken::USHORT || type == PlyToken::UINT16)
      {
      ...
      else if (type == PlyToken::INT || type == PlyToken::INT32)
      {
        fs.read((char*)&res[0], count * sizeof(int));
        if (big_endian_binary)
          big_to_little_endian((char*)&res[0], count, sizeof(int));
      }
      else if (type == PlyToken::UINT || type == PlyToken::UINT32)
      {
    ...
    {
      if (type == PlyToken::CHAR  || type == PlyToken::INT8  || type == PlyToken::UCHAR  || type == PlyToken::UINT8  || 
          type == PlyToken::SHORT || type == PlyToken::INT16 || type == PlyToken::USHORT || type == PlyToken::UINT16 ||
          type == PlyToken::INT   || type == PlyToken::UINT  || type == PlyToken::INT32  || type == PlyToken::UINT32)
      {
        for (int i = 0; i < count; ++i)
        {
          PlyToken token = NextToken();
          ...
          {
            printf("[ply_reader::ERROR] Got token (%d) instead of PlyToken::VALUE for ReadValues<int>\n", token.type);
            break;
          }
        }
      }
  ...
  {
    std::string name;
    PlyToken::Type val_type;
    PlyToken::Type index_type;
    bool is_list = false;
  };
  struct PlyElement
  {
  ...
  SimpleMesh LoadMeshFromPly(const std::string& path, bool verbose)
  {
    ...
    PlyToken token = tok.NextToken();
    // ply
    if (token.type != PlyToken::PLY)
    {
    ...
    token = tok.NextToken();
    if (token.type == PlyToken::ASCII)
    {
      tok.SetStateASCII();
    }
    else if (token.type == PlyToken::BINARY)
    {
      tok.SetStateBinary();
    }
    else
    ...
    token = tok.NextToken();
    while (token.type == PlyToken::COMMENT)
    {
      token = tok.NextToken();
      while (token.type == PlyToken::NAME)
        token = tok.NextToken();
    }

    while (token.type == PlyToken::ELEMENT)
    {
      //element vertex 8
      header.elements.push_back(PlyElement());
      token = tok.NextToken();
      if (token.type != PlyToken::NAME)
      {
      ...
      token = tok.NextToken();
      while (token.type == PlyToken::PROPERTY)
      {
        //property float32 x
        //property list uint8 int32 vertex_index
        ...
        if (token.type == PlyToken::LIST)
        {
          token = tok.NextToken();
          if (token.type < PlyToken::Type::CHAR || token.type >= PlyToken::Type::FLOAT)
          {
        ...
        if (token.type < PlyToken::Type::CHAR)
        {
    ...
    if (token.type != PlyToken::END_HEADER)
    {
    ...
    for (int i = 0; i < header.elements.size(); ++i)
    {
      ...
      // property float32 z
      bool vertex_format = 
          el.properties.size() == 3 &&
          el.properties[0].is_list == false &&
          el.properties[1].is_list == false && 
          el.properties[2].is_list == false &&
          el.properties[0].val_type >= PlyToken::Type::FLOAT &&
          el.properties[0].val_type == el.properties[1].val_type && 
          el.properties[0].val_type == el.properties[2].val_type;

      // property list uint8 int32 vertex_index
      bool face_format = 
          el.properties.size() == 1 && 
          el.properties[0].is_list &&
          el.properties[0].index_type >= PlyToken::Type::CHAR && 
          el.properties[0].index_type < PlyToken::Type::FLOAT &&
          el.properties[0].val_type >= PlyToken::Type::CHAR && 
          el.properties[0].val_type < PlyToken::Type::FLOAT;
    ...
    PlyToken::Type index_count_type = header.elements[faces_element_index].properties[0].index_type;
    PlyToken::Type index_type = header.elements[faces_element_index].properties[0].val_type;
