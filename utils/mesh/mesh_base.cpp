...
namespace cmesh4
{
  ...
  SimpleMesh CreateQuad(const int a_sizeX, const int a_sizeY, const float a_size)
  {
    ...
    SimpleMesh res(vertNum, quadsNum*2*3);
  ...
  float SimpleMesh::GetAvgTriArea() const
  {
    float res = 0.0;
  ...
  float SimpleMesh::GetAvgTriPerimeter() const
  {
    float res = 0.0;
    for(size_t i = 0; i < TrianglesNum(); i++)
    {
  ...
  void SimpleMesh::ApplyMatrix(const LiteMath::float4x4& m)
  {
